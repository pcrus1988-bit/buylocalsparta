...
for (const contract of ["requireAccountSession(request, true)", "customerDecideAskLocalOffer", 'body.action === "accept"', 'body.action === "decline"', "resolveCustomerPrivateOfferReference"]) if (!offerRoute.includes(contract)) failures.push(`Customer private-offer API is missing ${contract}`);
...
for (const contract of ["/api/account/ask-local/offers", 'decideOffer(actionReference, "accept")', 'decideOffer(actionReference, "decline")', "Αποδέχομαι την προσφορά", "requestReference, action"]) if (!client.includes(contract)) failures.push(`Customer Ask Local offer UI is missing ${contract}`);
...
console.log("Ask Local checks passed: auth/CSRF, validation, fair product routing, private vendor scope, request-scoped clarification lifecycle, clarification-gated offers, customer-owned reference-based offer decisions, notifications, live homepage and sitemap registry verified.");
