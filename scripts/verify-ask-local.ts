...
const service = read("apps/web/src/lib/ask-local-service.ts"),offerService = read("apps/web/src/lib/ask-local-offer-service.ts"),lifecycleService = read("apps/web/src/lib/ask-local-lifecycle-service.ts"),clarificationService = read("apps/web/src/lib/ask-local-clarification-service.ts"),route = read("apps/web/src/app/api/account/ask-local/route.ts"),offerRoute = read("apps/web/src/app/api/account/ask-local/offers/route.ts"),cancelRoute = read("apps/web/src/app/api/account/ask-local/cancel/route.ts"),clarificationRoute = read("apps/web/src/app/api/account/ask-local/clarifications/route.ts"),dailyOfferRoute = read("apps/web/src/app/api/daily/advice/offers/route.ts"),dailyReopenRoute = read("apps/web/src/app/api/daily/advice/offers/reopen/route.ts"),dailyClarificationRoute = read("apps/web/src/app/api/daily/advice/clarifications/route.ts"),page = read("apps/web/src/app/ask-local/page.tsx"),client = read("apps/web/src/components/AskLocalClient.tsx"),customerActions = read("apps/web/src/components/AskLocalCustomerActions.tsx"),clarificationClient = read("apps/web/src/components/AskLocalClarificationClient.tsx"),dailyClient = read("apps/web/src/components/VendorDailyAskLocalV2.tsx"),dailyPage = read("apps/web/src/app/daily/ask-local/page.tsx"),browserView = read("apps/web/src/lib/customer-ask-local-browser-view.ts"),home = read("apps/web/src/app/page.tsx"),homeSearch = read("apps/web/src/components/HomeQuickSearch.tsx"),searchDiscovery = read("apps/web/src/components/SearchDiscovery.tsx"),vendorOps = read("packages/postgres-runtime/src/vendor-operations.ts");
...
for (const contract of ["requireDailySession(request, true)", "vendorCreatePurchasableAskLocalOffer", "vendorAdviceWorkspace", "canonicalVariantId"]) if (!dailyOfferRoute.includes(contract)) failures.push(`Daily private-offer API is missing ${contract}`);
for (const contract of ["vendorCreatePurchasableAskLocalOffer", "vendorPurchasableAskLocalProducts", "customerCancelAskLocalRequest", "expireCustomerAskLocalOffers", "expireVendorAskLocalOffers", "counteroffer_allowed", "vo.status='approved'", "freshness_ttl_seconds", "assigned_offer_id", "status='expired'", "status='closed'", "customer_cancelled"]) if (!lifecycleService.includes(contract)) failures.push(`Ask Local lifecycle hardening is missing ${contract}`);
for (const contract of ["requireAccountSession(request, true)", "customerCancelAskLocalRequest", "customerAskLocalBrowserRequests"]) if (!cancelRoute.includes(contract)) failures.push(`Customer Ask Local cancellation API is missing ${contract}`);
for (const contract of ["expireCustomerAskLocalOffers", "customerAskLocalRequests(principal)"]) if (!browserView.includes(contract)) failures.push(`Customer Ask Local projection is missing ${contract}`);
for (const contract of ["/api/account/ask-local/offers", 'decideOffer(actionReference, "accept")', 'decideOffer(actionReference, "decline")', "Αποδέχομαι την προσφορά"]) if (!client.includes(contract)) failures.push(`Customer Ask Local offer UI is missing ${contract}`);
for (const contract of ["/api/account/ask-local/offers", "/api/account/ask-local/cancel", "Αποδοχή & checkout", "Ακύρωση αιτήματος", "/checkout/private-offer/"]) if (!customerActions.includes(contract)) failures.push(`Prominent customer Ask Local actions are missing ${contract}`);
for (const contract of ["/api/daily/advice/offers", "sendOffer(request.id", "canonicalVariantId", "Προϊόν που προσφέρεις", "Τιμή ανά τεμάχιο (€)", "Ισχύς προσφοράς", "Ανάκληση και νέα προσφορά"]) if (!dailyClient.includes(contract)) failures.push(`Daily Ask Local offer UI is missing ${contract}`);
for (const contract of ["expireVendorAskLocalOffers", "vendorPurchasableAskLocalProducts", "vendorAskLocalOfferStates", "VendorDailyAskLocalV2"]) if (!dailyPage.includes(contract)) failures.push(`Daily Ask Local page is missing ${contract}`);
for (const contract of ["requireDailySession(request, true)", "vendorReopenAskLocalOffer"]) if (!dailyReopenRoute.includes(contract)) failures.push(`Daily private-offer reopen API is missing ${contract}`);
...
for (const contract of ["/api/daily/advice/clarifications", "askClarification(request.id", 'request.status === "needs_info"', 'request.status === "awaiting_vendor"']) if (!dailyClient.includes(contract)) failures.push(`Vendor clarification UI is missing ${contract}`);

const dynamicHomepageAskLocal = home.includes("<HomeQuickSearch />")
  && homeSearch.includes("useSearchDiscovery(query, 12)")
  && searchDiscovery.includes("/api/search/suggest?q=")
  && homeSearch.includes("hasResults === false")
  && homeSearch.includes("/ask-local?need=")
  && homeSearch.includes("href={askLocalHref}");
...
console.log("Ask Local checks passed: auth/CSRF, fair routing, clarification gating, governed product-linked private offers, vendor-selected expiry, customer accept/decline/cancel, stale-offer expiration, protected checkout, notifications and sitemap registry verified.");
