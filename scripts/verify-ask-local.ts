...
const service = read("apps/web/src/lib/ask-local-service.ts"),route = read("apps/web/src/app/api/account/ask-local/route.ts"),page = read("apps/web/src/app/ask-local/page.tsx"),client = read("apps/web/src/components/AskLocalClient.tsx"),home = read("apps/web/src/app/page.tsx"),vendorOps = read("packages/postgres-runtime/src/vendor-operations.ts"),sitemap = read("apps/web/src/app/sitemap.ts");
...
if (!sitemap.includes("`${origin}/ask-local`")) failures.push("Sitemap must contain Ask Local");
if (failures.length) { console.error("Ask Local checks failed:\n" + failures.map((failure) => `- ${failure}`).join("\n")); process.exit(1); }
console.log("Ask Local checks passed: auth/CSRF, validation, fair product routing, private vendor scope, customer ownership, live homepage and sitemap verified.");
