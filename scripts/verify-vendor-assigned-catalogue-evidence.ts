...
const migration = read("db/migrations/0165_vendor_assigned_catalogue_evidence.sql");
...
]) expect(migration.includes(contract), `Schema 165 is missing assigned-catalogue evidence contract: ${contract}`);
for (const forbidden of ["catalog_attribute_mapping_rules", "apply_catalog_attribute_mapping_rule", "catalog_attribute_normalize_key"]) {
  expect(!migration.includes(forbidden), `Schema 165 must not reintroduce obsolete attribute mapper logic: ${forbidden}`);
}
...
expect(runtime.includes("EXPECTED_SCHEMA_VERSION = 165"), "PostgreSQL runtime must require schema 165");
