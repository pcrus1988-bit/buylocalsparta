...
for (const route of INDEXABLE_STATIC_ROUTES) {
  ...
  if (route.href !== "/" && !source.includes(`canonical: "${route.href}"`)) failures.push(`${route.href} is missing canonical metadata`);
...
for (const absolutePath of linkSources) {
  ...
  for (const match of source.matchAll(/href\s*=\s*"(\/[^\"]*)"/g)) {
    ...
    if (pathname.startsWith("/api/")) {
      failures.push(`${displayPath} exposes API route ${pathname} as a navigation link`);
      continue;
    }
...
console.log(`Public navigation checks passed: ${pageRoutes.length} App Router pages classified, literal links resolved, responsive navigation and sitemap registry verified.`);
