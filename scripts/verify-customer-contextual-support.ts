...
const migration = read("db/migrations/0105_customer_contextual_support.sql");
...
expect(checksums["0105_customer_contextual_support.sql"] === migrationHash, `0105 checksum mismatch: manifest=${checksums["0105_customer_contextual_support.sql"] ?? "missing"} actual=${migrationHash}`);
expect(postgresRuntime.includes("EXPECTED_SCHEMA_VERSION = 105"), "PostgreSQL readiness must expect schema version 105");
