...
for (const contract of [
  "customerPrivateOfferActionToken(principal.userId, id)",
...
for (const contract of [
  ...
  "if (id !== offer.actionReference) redirect",
  "/checkout/private-offer/${encodeURIComponent(offer.actionReference)}"
...
for (const contract of [
  "encodeURIComponent(offer.actionReference)}/checkout",
  "href={`/account/orders/${encodeURIComponent(createdOrderReference)}`}"
