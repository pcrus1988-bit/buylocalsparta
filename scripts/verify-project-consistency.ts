...
const molliePackage = readJson("packages/mollie-payments/package.json");
...
if (postgresPackage.dependencies?.["@buy-local-sparta/mollie-payments"] !== molliePackage.version || webPackage.dependencies?.["@buy-local-sparta/mollie-payments"] !== molliePackage.version) errors.push("Web/PostgreSQL runtime Mollie dependency must match the local Mollie package version");
if ([molliePackage, postgresPackage, myDataPackage, objectStoragePackage, mediaProcessingPackage, searchPackage, resendPackage, boxNowPackage].some((pkg) => pkg.version !== rootPackage.version)) errors.push("Release-scoped production adapter versions must match the root build version");
...
if (!rootPackage.scripts?.["test:mollie"] || !rootPackage.scripts?.["typecheck:mollie"]) errors.push("Root scripts must include Mollie provider tests and TypeScript typecheck");
...
if (!nextConfig.includes('"@buy-local-sparta/mollie-payments"')) errors.push("Next.js must transpile the local Mollie payments workspace");
...
if (!readyRoute.includes("productionDatabaseReadiness") || !readyRoute.includes("const ok = database.ok") || !readyRoute.includes("status: ok ? 200 : 503")) errors.push("Production readiness route must gate on PostgreSQL/Mollie readiness and return HTTP 503 when unavailable");
const workerSource = read("workers/postgres-worker.ts");
for (const workerBoundary of ["PostgreSQL worker refused to start", "inventory.reservation_expiry", "payments.mollie_reconciliation_watch", "retention.security_events", "retention.analytics_events", "runtime.persistence.scheduledJobs"]) {
...
for (const envKey of ["DATABASE_URL=", "BLS_DB_POOL_MAX=", "BLS_DB_CONNECT_TIMEOUT_MS=", "BLS_DB_IDLE_TIMEOUT_MS=", "BLS_WORKER_POLL_MS=", "BLS_ALLOW_DATABASELESS_PREVIEW=", "MOLLIE_PAYMENTS_ENABLED=", "MOLLIE_API_KEY=", "MOLLIE_API_BASE_URL=", "MOLLIE_PUBLIC_BASE_URL=", "MOLLIE_REQUEST_TIMEOUT_MS=", "BLS_ALLOW_MOLLIE_TEST_PREVIEW=", "BLS_MYDATA_ISSUANCE_ENABLED=", "BLS_MYDATA_MAPPING_VERSION=", "AADE_MYDATA_ENVIRONMENT=", "AADE_MYDATA_BASE_URL=", "AADE_MYDATA_USER_ID=", "AADE_MYDATA_SUBSCRIPTION_KEY=", "AADE_MYDATA_SPEC_VERSION=", "BLS_SEARCH_ENABLED=", "MEILISEARCH_URL=", "MEILISEARCH_INDEX_UID=", "MEILISEARCH_SEARCH_KEY=", "MEILISEARCH_ADMIN_KEY=", "BLS_EMAIL_DELIVERY_ENABLED=", "RESEND_API_KEY=", "RESEND_FROM=", "RESEND_WEBHOOK_SECRET=", "BLS_NOTIFICATION_SUPPRESSION_SECRET="]) {
...
const catalogViewBase = read("apps/web/src/lib/catalog-view-base.ts");
...
if (!catalogView.includes("cv.active=true") && !catalogViewBase.includes("canonicalIsPubliclyAllowed")) errors.push("Public catalog must suppress inactive/recalled/compliance-held canonical products before fairness assignment");
...
if (!(catalogView.includes("production.search.search") || catalogViewBase.includes("production.search.search")) || !(catalogView.includes('reason: "search_card"') || catalogViewBase.includes('reason: "search_card"'))) errors.push("Production customer search must query the external canonical index before Fair Vendor Exposure assignment");
...
if (!read("packages/postgres-runtime/src/mollie-payments.ts").includes("order.payment_confirmed") || !read("packages/postgres-runtime/src/mollie-payments.ts").includes("order.refund_completed")) errors.push("Captured/refunded customer payments must enqueue durable transactional notifications when email delivery is enabled");
...
for (const boundary of ["MolliePaymentsClient", "requestTransmittedDocs", "ResendEmailProvider", "S3ObjectStorage", "ClamAvScanner", "boxNowShipping", "/api/health/ready", "--record"]) if (!activationPreflight.includes(boundary)) errors.push(`Staging preflight must retain ${boundary} readiness boundary`);
...
for (const proof of ["crossInstanceProductionSearchProjection", "crossInstanceResendDeliveryWebhook", "crossInstancePersistentCart", "idempotentCheckout", "concurrentOversellProtection", "crossInstanceCustomerOrders", "pendingPaymentExpiryCleanup", "crossInstanceVendorSession", "crossInstanceVendorInventory", "crossInstanceVendorCatalog", "crossInstanceVendorFulfilment", "mollieCrossInstancePayment", "mollieVerifiedPaymentConfirmation", "molliePaidReservationProtection", "mollieRefundCancellation", "mollieWebhookReconciliationIdempotency", "mollieCanonicalStateMonotonicity", "mollieLateCaptureAutoRefund", "vendorTenantIsolation", "crossInstanceAdminSession", "crossInstanceAdminCategoryGovernance", "crossInstanceAdminCms", "crossInstanceAdminRecall", "adminSettlementMakerChecker", "crossInstanceAdminAudit", "crossInstanceActivationEvidence"]) {
...
if (!postgresAdminGovernance.includes("route approved refunds through the configured Mollie payments orchestration service") || !adminGovernance.includes("executeApprovedReturnRefund")) errors.push("Direct Admin refund mutation must stay fail-closed while the web orchestration routes approved money movement through Mollie");
