...
INSERT INTO vendor_family_variant_context(baseline_offer_count,baseline_variant_count)
SELECT (SELECT count(*) FROM public.vendor_offers),(SELECT count(*) FROM public.canonical_variants);
...
SET sibling_id=:'sibling_id',sibling_public_id=:'sibling_public_id',family_id=:'family_id';
...
BEGIN
  ...
  SELECT active,family_id INTO anchor FROM public.canonical_variants WHERE id=:'anchor_id';
  ...
  END IF;
  IF family.product_type_id<>:'product_type_id'::uuid THEN
...
BEGIN
  SELECT * INTO c FROM vendor_family_variant_context LIMIT 1;
  IF :'repeat_sibling_id'::uuid<>c.sibling_id OR :'repeat_disposition'<>'linked_existing' THEN
    RAISE EXCEPTION 'repeat family variant did not reuse sibling: %, %',:'repeat_sibling_id',:'repeat_disposition';
  END IF;
...
DECLARE v record;
BEGIN
  SELECT active,gtin,family_id INTO v FROM public.canonical_variants WHERE id=:'sibling_1024_id';
  IF :'sibling_1024_disposition'<>'created_sibling' OR v.active<>false OR v.gtin<>'5012345678900' THEN
    RAISE EXCEPTION 'valid unseen GTIN sibling creation failed: %, %, %',:'sibling_1024_disposition',v.active,v.gtin;
  END IF;
  IF NOT EXISTS(
    SELECT 1 FROM public.product_identifiers
    WHERE canonical_variant_id=:'sibling_1024_id' AND normalized_value='5012345678900'
...
BEGIN
  SELECT * INTO c FROM vendor_family_variant_context LIMIT 1;
  IF :'conflict_disposition'<>'review' OR :'conflict_reason'<>'gtin_family_conflict' THEN
    RAISE EXCEPTION 'outside-family GTIN did not fail closed: %, %',:'conflict_disposition',:'conflict_reason';
  END IF;
  IF EXISTS(
...
BEGIN
  BEGIN
    PERFORM * FROM bls_private.ensure_vendor_family_variant(:'anchor_id',:'missing_required_submission_id','computer_storage','{}'::jsonb,NULL);
...
BEGIN
  ...
  IF EXISTS(
    SELECT 1 FROM public.canonical_variants
    WHERE family_id=c.family_id AND id<>:'anchor_id' AND active=true
