...
const migration = read("db/migrations/0217_hub_expansion_prospects.sql");
const checksum = JSON.parse(read("db/migrations/checksums.0217.json")) as Record<string, string>;
...
requireText(migration, "CREATE TABLE IF NOT EXISTS hub_expansion_prospects", "Migration 0217 must create prospect storage");
...
requireText(postgresRuntime, "EXPECTED_SCHEMA_VERSION = 217", "PostgreSQL runtime schema head must be 217");

const migrationHash = createHash("sha256").update(migration).digest("hex");
if (checksum["0217_hub_expansion_prospects.sql"] !== migrationHash) {
  errors.push(`Migration 0217 checksum mismatch: expected ${migrationHash}, manifest has ${checksum["0217_hub_expansion_prospects.sql"] ?? "missing"}`);
}
