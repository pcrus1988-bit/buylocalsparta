...
const page = read("apps/web/src/app/advice/page.tsx"), product = read("apps/web/src/app/product/[id]/page.tsx"), productVendor = read("apps/web/src/components/ProductVendorHumanCard.tsx"), vendor = read("apps/web/src/app/vendor/[id]/page.tsx"), vendorAsk = read("apps/web/src/components/VendorAskLocalPanel.tsx");
...
const productContextPreserved = product.includes("/ask-local?product=")
  || (product.includes("<ProductVendorHumanCard productId={product.id}") && productVendor.includes("/ask-local?product=${encodeURIComponent(productId)}"));
if (!productContextPreserved) failures.push("Product advice calls must preserve product context in the live Ask Local workflow");
