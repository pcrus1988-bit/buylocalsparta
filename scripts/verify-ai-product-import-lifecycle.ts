...
const migration = read("db/migrations/0125_ai_product_import_persistence.sql");
...
for (const table of ["catalog_import_mapping_profiles", "catalog_import_runs", "catalog_import_row_decisions"]) {
  expect(migration.includes(`CREATE TABLE public.${table}`), `Migration 0125 must create ${table}`);
...
expect(runtime.includes("EXPECTED_SCHEMA_VERSION = 125"), "PostgreSQL runtime must require schema 125");
