...
-- A. Exact valid GTIN is the strongest identity and must reuse the canonical.
...
VALUES(
  ...
  jsonb_build_object('mpn','MPN-18','gtin','0195949052637','variantAttributes',jsonb_build_object('size','42','color','Black')),
...
DO $$
DECLARE d text; r text; v uuid; c catalog_identity_context%ROWTYPE;
BEGIN
  SELECT * INTO c FROM catalog_identity_context LIMIT 1;
  SELECT disposition,reason_code,existing_variant_id INTO d,r,v
  FROM bls_private.catalog_source_canonicalization_preview('catalog-identity-ci',:'exact_snapshot_id',0.95)
...
DO $$
DECLARE c catalog_identity_context%ROWTYPE; result jsonb;
BEGIN
  SELECT * INTO c FROM catalog_identity_context LIMIT 1;
  SELECT e.result INTO result FROM exact_result e LIMIT 1;
...
-- B. The same valid GTIN with conflicting material attributes is a real Admin exception.
INSERT INTO public.catalog_source_snapshots(source_id,source_hash,source_version,observed_at,row_count)
...
BEGIN
  ...
  FROM bls_private.catalog_source_canonicalization_preview('catalog-identity-ci',:'conflict_snapshot_id',0.95)
...
-- C. Same brand/style with a materially different size is a distinct canonical variant.
INSERT INTO public.catalog_source_snapshots(source_id,source_hash,source_version,observed_at,row_count)
...
VALUES(
  :'variant_snapshot_id',:'source_id',:'source_taxonomy_node_id','MPN-43','MPN-43','Example Tools DT-18 size 43',
  ...
  jsonb_build_object('mpn','MPN-18','variantAttributes',jsonb_build_object('size','43','colour','Black')),
  jsonb_build_object('fixture',true)
)
