...
export const config = {
  // Public catalogue media/search helper routes are deliberately excluded from
  // middleware. They never use visitor identity, and avoiding middleware cookies
  // allows their explicit CDN cache headers to work across visitors.
  matcher: ["/((?!_next/static|_next/image|favicon.ico|api/health|api/media/|api/catalog-source-image/|api/catalog/msrp|api/search/suggest).*)"]
};
