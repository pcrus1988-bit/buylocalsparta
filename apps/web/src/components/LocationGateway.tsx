...
type LocationGatewayProps = Readonly<{ hubs?: readonly ExpansionHub[] }>;
...
function readSavedLocality(hubs: readonly ExpansionHub[]): ExpansionHub | undefined {
  try {
    const raw = window.localStorage.getItem(STORAGE_KEY);
    if (!raw) return undefined;
    const parsed = JSON.parse(raw) as { slug?: string };
    return parsed.slug ? hubs.find((hub) => hub.slug === parsed.slug) : undefined;
  } catch {
    return undefined;
  }
}
...
export function LocationGateway({ hubs = EXPANSION_HUBS }: LocationGatewayProps) {
  ...
  useEffect(() => {
    const saved = readSavedLocality(hubs);
    if (saved) {
  ...
  }, [distanceByHubId, hubs, query, regionCode, userCoordinates]);
