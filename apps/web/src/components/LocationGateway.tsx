...
type ExpansionHubDisplayState = "inactive" | "prospect" | "active";

export type LocationGatewayRuntimeHub = Readonly<{
  hubId: string;
...
type RuntimeIndex = ReadonlyMap<string, LocationGatewayRuntimeHub>;
...
function runtimeFor(runtimeByHubId: RuntimeIndex, hub: ExpansionHub): LocationGatewayRuntimeHub | undefined {
  return runtimeByHubId.get(hub.id);
}

function hubState(runtimeByHubId: RuntimeIndex, hub: ExpansionHub): ExpansionHubDisplayState {
  return runtimeFor(runtimeByHubId, hub)?.lifecycleState ?? "inactive";
}

function prospectCount(runtimeByHubId: RuntimeIndex, hub: ExpansionHub): number | undefined {
  const runtime = runtimeFor(runtimeByHubId, hub);
  return runtime && runtime.prospectCount > 0 ? runtime.prospectCount : undefined;
}

function hubStateLabel(runtimeByHubId: RuntimeIndex, hub: ExpansionHub): string {
  const state = hubState(runtimeByHubId, hub);
  if (state === "active") return "Ενεργή";
  if (state === "prospect") return "Με prospect vendors";
  return "Ανενεργή";
}

function hubStateDescription(runtimeByHubId: RuntimeIndex, hub: ExpansionHub): string {
  const state = hubState(runtimeByHubId, hub);
  if (state === "active") return "Ενεργή τοπική αγορά";
  if (state === "prospect") {
    const count = prospectCount(runtimeByHubId, hub);
...
export function LocationGateway({ runtimeHubs }: Readonly<{ runtimeHubs: readonly LocationGatewayRuntimeHub[] }>) {
  const runtimeByHubId = useMemo(() => new Map(runtimeHubs.map((hub) => [hub.hubId, hub] as const)), [runtimeHubs]);
  ...
  const filteredHubs = useMemo(() => {
    ...
    return EXPANSION_HUBS
      ...
      .sort((a, b) => {
        ...
        const stateOrder = HUB_STATE_ORDER[hubState(runtimeByHubId, a.hub)] - HUB_STATE_ORDER[hubState(runtimeByHubId, b.hub)];
  ...
  }, [distanceByHubId, query, regionCode, runtimeByHubId, userCoordinates]);
  ...
  useEffect(() => {
    ...
    hubLayersRef.current = visibleMapHubs.map((hub) => {
      ...
      const tooltipStatus = state === "prospect" && count
        ? `${hubStateLabel(runtimeByHubId, hub)} · ${count}`
        : hubStateLabel(runtimeByHubId, hub);
      layer
  ...
  }, [mapState, runtimeByHubId, selectedHub?.id, visibleMapHubs]);
  ...
  useEffect(() => {
    ...
    const stateColor = HUB_STATE_COLORS[hubState(runtimeByHubId, selectedHub)];
  ...
  }, [mapState, runtimeByHubId, selectedHub]);
  ...
  function confirmHub(hub: ExpansionHub) {
    ...
    if (state === "prospect") {
      const count = prospectCount(runtimeByHubId, hub);
      setSavedMessage(`${hub.nameEl}: η περιοχή αποθηκεύτηκε. Το hub είναι υπό ανάπτυξη${count ? ` με ${count} prospect vendors` : " με prospect vendors"}.`);
      return;
    }
  ...
  const selectedState = selectedHub ? hubState(runtimeByHubId, selectedHub) : undefined;
          ...
          {selectedHub && selectedState ? (
            <div className={styles.mapSelection}>
              <div>
                ...
                <span>{coverageLabel(selectedHub)} · {hubStateDescription(runtimeByHubId, selectedHub)}</span>
              </div>
              <span
                style={{
          ...
          {selectedHub && selectedState ? (
            <div className={`${styles.selectedCard} ${selectedState === "active" ? styles.selectedCardLive : ""}`}>
              <div className={styles.selectedHeader}>
                ...
                >{hubStateLabel(runtimeByHubId, selectedHub)}</span>
              </div>
              <p>{hubStateDescription(runtimeByHubId, selectedHub)}</p>
              ...
              return (
                <button key={hub.id} type="button" className={`${styles.hubRow} ${selectedHub?.id === hub.id ? styles.hubRowSelected : ""}`} onClick={() => chooseHub(hub)}>
                  ...
                  <span className={styles.rowCopy}>
                    ...
                    <small>{hub.regionEl} · {hubStateDescription(runtimeByHubId, hub)}</small>
                  </span>
