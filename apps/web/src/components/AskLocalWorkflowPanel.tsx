...
export function AskLocalWorkflowPanel({ requests, vendors, csrfToken, canManage }: {
  ...
  const filtered = useMemo(() => {
    ...
    return requests.filter((request) => (owner === "all" || request.workflowOwnerKind === owner) && (!q || [request.referenceNumber, request.need, request.voiceTranscript, request.barcode, request.customerName, request.customerEmail, request.assignedVendorName, request.category, request.postcode].some((value) => String(value ?? "").toLocaleLowerCase("el-GR").includes(q))));
  ...
  return <>
    ...
    {filtered.length === 0 ? <div className="workspace-inline-note">No Ask Local requests match the current filters.</div> : <div className="admin-work-queue-split">
      <div className="admin-work-list" aria-label="Ask Local queue">{filtered.map((request) => {
        ...
        return <button type="button" className={`admin-work-list-row${selected?.id === request.id ? " is-selected" : ""}${overdue ? " is-overdue" : ""}`} key={request.id} onClick={() => setSelectedId(request.id)}><span><strong>{request.need}</strong><small>{request.referenceNumber} · {request.customerName}{request.barcode ? ` · ${request.barcode}` : ""}</small></span><span><b>{request.workflowOwnerKind === "vendor" ? request.assignedVendorName ?? "Vendor" : "Admin"}</b><small>{overdue ? "OVERDUE" : request.responseDueAt ? `Due ${dateTime(request.responseDueAt)}` : request.status}</small></span></button>;
      })}</div>
      {selected ? <article className="admin-work-detail">
        <div className="admin-work-detail-head"><div><span>Ask Local 2.0 · {selected.captureSource ?? "text"}</span><h2>{selected.need}</h2><p>{selected.referenceNumber} · created {dateTime(selected.createdAt)}</p></div><span className="status-pill">{selected.workflowOwnerKind === "vendor" ? "Vendor-owned" : "Admin-owned"}</span></div>
        <div className="admin-decision-summary"><div><span>Status</span><strong>{selected.status}</strong></div><div><span>Owner</span><strong>{selected.assignedVendorName ?? selected.assignedAdminId ?? "Admin queue"}</strong></div><div><span>Deadline</span><strong>{selected.responseDueAt ? dateTime(selected.responseDueAt) : "—"}</strong></div></div>
        {selected.referenceImageDataUrl ? <div className="ask-local-context"><Image src={selected.referenceImageDataUrl} alt={`Ιδιωτική φωτογραφία αναφοράς ${selected.referenceNumber}`} width={420} height={315} unoptimized /></div> : null}
        {selected.voiceTranscript ? <div className="workspace-inline-note"><strong>Voice:</strong> {selected.voiceTranscript}</div> : null}
        <div className="workspace-compact-list">
          ...
          <div className="workspace-compact-row"><strong>Request</strong><span>qty {selected.quantity} · {selected.postcode}</span><small>{selected.category ?? "No category context"}</small></div>
          {selected.barcode ? <div className="workspace-compact-row"><strong>Barcode / model</strong><span>{selected.barcode}</span><small>Customer-supplied or browser-detected — verify before matching.</small></div> : null}
          <div className="workspace-compact-row"><strong>Assignment reason</strong><span>{selected.assignmentReason ?? "—"}</span></div>
        </div>
        <div className="workspace-action-bar"><span>Workflow ownership</span><div className="workspace-action-buttons"><Link className="button button-secondary" href={`/admin/customers/${encodeURIComponent(selected.customerId)}`}>Customer 360</Link>{canManage && selected.workflowOwnerKind !== "admin" ? <AdminActionButton label="Return to Admin" endpoint="/api/admin/ask-local" csrfToken={csrfToken} body={{ requestId: selected.id, owner: "admin" }} reasonPrompt="Reason for returning this Ask Local request to Admin triage" /> : null}</div></div>
        {canManage ? <div className="admin-assignment-row"><select aria-label={`Vendor for ${selected.referenceNumber}`} value={vendorSelection[selected.id] ?? selected.assignedVendorId ?? ""} onChange={(event) => setVendorSelection((current) => ({ ...current, [selected.id]: event.target.value }))}><option value="">Choose eligible vendor…</option>{vendors.map((vendor) => <option value={vendor.id} key={vendor.id}>{vendor.name}</option>)}</select><button type="button" className="button button-secondary" disabled={busyId === selected.id} onClick={() => assignVendor(selected.id)}>{busyId === selected.id ? "…" : selected.workflowOwnerKind === "vendor" ? "Reassign vendor" : "Assign vendor"}</button></div> : null}
        {error[selected.id] ? <small className="form-error" role="alert">{error[selected.id]}</small> : null}
      </article> : null}
    </div>}
  </>;
}
