  ...
  return <div className={styles.grid}>
    ...
    <section><div className={styles.sectionTitle}><div><div className={styles.eyebrow}>Assigned</div><h2>Οι εργασίες μου</h2></div><button className={styles.buttonSecondary} type="button" onClick={()=>void refresh()}>Ανανέωση</button></div>
      {workspace.assigned.length===0?<div className={styles.empty}>Δεν έχεις ανατεθειμένες εργασίες.</div>:<div className={`${styles.grid} ${styles.two}`}>{workspace.assigned.map((job)=><article className={styles.card} key={job.id}>
        ...
        <div className={styles.toolbar}><button className={styles.button} type="button" onClick={()=>setScanner(true)}>Σάρωση επιβεβαίωσης</button><button className={styles.buttonSecondary} type="button" onClick={()=>void toggleTracking(job)} disabled={busy===`tracking:${job.id}`}>{job.liveTracking?"Διακοπή live tracking":"Έναρξη live tracking"}</button></div>
    ...
    <section><div className={styles.sectionTitle}><div><div className={styles.eyebrow}>Dispatcher</div><h2>Προτάσεις ανάθεσης</h2></div></div>{workspace.available.length===0?<div className={styles.empty}>Δεν υπάρχει ενεργή πρόταση ανάθεσης. Ο dispatcher αξιολογεί αυτόματα νέες εργασίες και τις τρέχουσες διαδρομές.</div>:<div className={`${styles.grid} ${styles.three}`}>{workspace.available.map((job)=><article className={styles.card} key={job.id}><span className={styles.badge}>{job.type==="outbound"?"Παράδοση":"Επιστροφή"}</span><h3>{job.orderId}</h3><p className={styles.muted}>{job.stops.length} σημεία · η πρόταση δημιουργήθηκε από τον αυτόματο dispatcher. Οι διευθύνσεις πελατών εμφανίζονται μετά την αποδοχή.</p><div className={styles.grid}><button className={styles.button} type="button" disabled={Boolean(busy)} onClick={()=>void action("accept_offer",{jobId:job.id})}>Αποδοχή ανάθεσης</button><select value={declineReasons[job.id]??""} onChange={(event)=>setDeclineReasons((current)=>({...current,[job.id]:event.target.value}))}><option value="">Λόγος απόρριψης…</option>{DECLINE_REASONS.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><button className={styles.buttonSecondary} type="button" disabled={Boolean(busy)||!(declineReasons[job.id]??"")} onClick={()=>void action("decline_offer",{jobId:job.id,reason:declineReasons[job.id]})}>Απόρριψη & επανεκτίμηση</button></div></article>)}</div>}</section>
