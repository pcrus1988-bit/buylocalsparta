...
type Draft = { quantity: number; reason: string; remedy: "refund" | "replacement" | "repair"; note: string };
...
const remedyLabel: Record<string, string> = { refund: "Επιστροφή χρημάτων", replacement: "Αντικατάσταση", repair: "Επισκευή" };
...
function initialDraft(maxQuantity: number): Draft {
  return { quantity: Math.min(1, maxQuantity), reason: "withdrawal", remedy: "refund", note: "" };
}
...
function returnLifecycle(item: CustomerReturnCaseView): readonly CustomerLifecycleStage[] {
  ...
  return labels.map((label, index) => {
    ...
    const action = index === 2 && Boolean(item.rmaCode || item.instructions || item.returnByAt);
    return { label, state: action ? "action" as const : "current" as const };
  });
}
...
export function CustomerReturnsPanel({ orderId, csrfToken, initialLines, initialReturns }: {
  ...
  function patchDraft(lineId: string, patch: Partial<Draft>) {
    setDrafts((current) => ({ ...current, [lineId]: { ...(current[lineId] ?? initialDraft(lines.find((line) => line.id === lineId)?.returnableQuantity ?? 1)), ...patch } }));
  }
  ...
  async function submitReturn(line: CustomerReturnLine) {
    ...
    try {
      ...
      setDrafts((current) => ({ ...current, [line.id]: initialDraft(payload.lines.find((item) => item.id === line.id)?.returnableQuantity ?? 0) }));
  ...
  return <div className="order-detail-card is-refined customer-returns-panel">
    <div className="customer-return-heading"><div><div className="eyebrow">Μετά την αγορά</div><h2>Επιστροφές & λύσεις</h2></div><CustomerHowItWorks title="Πώς λειτουργεί η επιστροφή;"><p>Διάλεξε το συγκεκριμένο προϊόν, ποσότητα, λόγο και τη λύση που προτιμάς. Η επιλογή σου είναι αίτημα — η τελική επιλεξιμότητα και η εγκεκριμένη λύση ελέγχονται από τη διαδικασία επιστροφών. Μην αποστείλεις προϊόν πριν εμφανιστούν επίσημες οδηγίες ή RMA.</p></CustomerHowItWorks></div>
    ...
    {returnableLines.length > 0 && <section className="customer-return-request-list" aria-labelledby="return-request-title">
      ...
      {returnableLines.map((line) => {
        ...
        return <details className="customer-return-request" key={line.id}>
          <summary><span><strong>{line.title}</strong><small>{line.returnableQuantity} {line.returnableQuantity === 1 ? "τεμάχιο διαθέσιμο" : "τεμάχια διαθέσιμα"} για αίτημα</small></span><span>Έναρξη →</span></summary>
    ...
    {returns.length > 0 && <section className="customer-return-cases" aria-labelledby="return-cases-title">
      ...
      {returns.map((item) => {
        ...
        return <article className="customer-return-case" key={item.id}>
          ...
          {actionNeeded ? <CustomerStatusNotice tone="action" title="Χρειάζεται ενέργεια από εσένα"><p>{item.instructions ?? "Ακολούθησε τις οδηγίες επιστροφής που εμφανίζονται για αυτό το αίτημα."}</p>{item.rmaCode && <p><strong>RMA:</strong> {item.rmaCode}</p>}{item.returnByAt && <p><strong>Ολοκλήρωσε την αποστολή έως:</strong> {date(item.returnByAt)}</p>}</CustomerStatusNotice> : ["requested", "received", "inspected", "remedy_approved"].includes(item.status) ? <CustomerStatusNotice tone="progress" title="Δεν χρειάζεται ενέργεια από εσένα τώρα"><p>{returnStatusLabel[item.status] ?? item.status}. Θα ενημερωθείς όταν αλλάξει το επόμενο βήμα.</p></CustomerStatusNotice> : null}
