...
export function AdminSeoIssueQueue({ history, csrfToken, canWrite, pageIdsByRoute = {}, embedded = false }: {
  history: SeoCrawlHistorySnapshot;
  csrfToken: string;
  canWrite: boolean;
  pageIdsByRoute?: Readonly<Record<string, string>>;
  embedded?: boolean;
}) {
  const content = <>
    <WorkspaceSectionHeading eyebrow="Issue lifecycle" title="Recurring URL-level SEO problems" note="Each market + route + issue code has one durable fingerprint. Fix the underlying cause, run a targeted production recheck, and let clean evidence auto-resolve open findings. Ignored findings remain suppressed until an operator changes their lifecycle state." />
    ...
    {!history.persistenceAvailable
      ...
      : history.issues.length === 0
        ...
        : <div className="workspace-queue-list">{history.issues.map((issue) => {
            const guidance = seoIssueGuidance(issue.code);
            const pageId = pageIdsByRoute[issue.route];
            return <article className="workspace-queue-card" key={issue.id}>
              ...
              <div className="workspace-compact-list" style={{ marginTop: 10 }}>
                ...
                <div className="workspace-compact-row"><strong>Next verification</strong><span>{guidance.nextStep}</span></div>
              ...
              <div className="workspace-action-bar"><span><Link className="text-link" href={issue.route} target="_blank">Open public page ↗</Link>{pageId ? <> · <Link className="text-link" href={`/admin/seo/pages/${encodeURIComponent(pageId)}`}>Open SEO record →</Link></> : null}</span>{canWrite && <div className="workspace-action-buttons">
                <AdminActionButton label="Recheck production" endpoint="/api/admin/seo/crawl/recheck" csrfToken={csrfToken} body={{ route: issue.route }} />
                ...
                {issue.status !== "resolved" && <AdminActionButton label="Resolve manually" endpoint="/api/admin/seo/crawl/issues/action" csrfToken={csrfToken} body={{ issueId: issue.id, action: "resolve" }} reasonPrompt="Resolution evidence or reason. Prefer a clean recheck when possible. Minimum 5 characters." />}
