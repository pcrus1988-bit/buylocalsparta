...
import { groupExplicitSizeFacets } from "../lib/catalog-size-facets";
...
export function VendorCatalogBrowser({ products, vendor, demoVendorId, vendorId }: {
  ...
  const sizeFacets = useMemo(
    () => remoteFacets?.sizes ?? unique(products.flatMap((product) => product.sizes)).map((value) => ({ value, label: value, count: 0 })),
    [products, remoteFacets]
  );
  const sizes = useMemo(() => sizeFacets.map((entry) => entry.value), [sizeFacets]);
  const groupedSizes = useMemo(() => groupExplicitSizeFacets(sizeFacets), [sizeFacets]);
    ...
    <div className="vc-filter-panel">
      ...
      <div className="vc-filter-section">
        ...
        <div className="vc-fields">
          ...
          {sizes.length > 1 ? <label><span>Μέγεθος</span><select value={size} onChange={(event) => setSize(event.target.value)}><option value="all">Όλα</option>{groupedSizes ? groupedSizes.map((group) => <optgroup label={group.label} key={group.key}>{group.options.map((option) => <option value={option.value} key={option.value}>{option.label}{typeof option.count === "number" && option.count > 0 ? ` (${option.count})` : ""}</option>)}</optgroup>) : sizes.map((value) => <option value={value} key={value}>{value}</option>)}</select>{groupedSizes ? <small className="vc-size-note">EU / US / UK εμφανίζονται χωριστά μόνο όταν το σύστημα δηλώνεται στα δεδομένα.</small> : null}</label> : null}
