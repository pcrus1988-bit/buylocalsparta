...
export function OrderDetailClient({ initial }: { initial: Detail }) {
  ...
  return <section className="shell order-detail-grid is-refined">
    ...
    <aside className="order-detail-side">
      <div className="order-summary"><h2>Σύνοψη</h2><div className="summary-row"><span>Εμπορεύματα</span><strong>{data.merchandiseSubtotal}</strong></div><div className="summary-row"><span>Παράδοση</span><strong>{data.deliveryCharge}</strong></div><div className="summary-row"><span>Έκπτωση</span><strong>{data.discount}</strong></div><div className="summary-row"><span>Σύνολο</span><strong>{data.total}</strong></div>{data.cancellationReason && <div className="workspace-inline-note">Ακύρωση: {data.cancellationReason}{data.cancelledAt ? ` · ${date(data.cancelledAt)}` : ""}</div>}</div>
      <div className="order-help-links" aria-label="Βοήθεια παραγγελίας"><Link href="/delivery-pickup"><span>Παράδοση & παραλαβή</span><span aria-hidden="true">→</span></Link><Link href="/returns-refunds"><span>Πολιτική επιστροφών</span><span aria-hidden="true">→</span></Link></div>
