...
export function VendorReturnsClient({ initial }: { initial: Workspace }) {
  ...
  const activeCases = initial.returns.filter((item) => !["closed", "refunded", "rejected"].includes(item.status)).length;
  ...
  return <>
    ...
    <section className="shell vendor-section">
      <WorkspaceSectionHeading eyebrow="Assigned cases" title="Returns & after-sales" note="Η πλατφόρμα αποφασίζει eligibility και οικονομικό remedy. Το κατάστημα επιβεβαιώνει τη φυσική παραλαβή/κατάσταση του προϊόντος και εκτελεί μόνο τις ανατεθειμένες εργασίες. Refund μέσω Viva εκτελείται αποκλειστικά από εξουσιοδοτημένο admin." />
