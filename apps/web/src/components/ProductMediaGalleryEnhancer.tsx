  ...
  useEffect(() => {
    ...
    fetch(`/api/catalog-source-gallery/${encodeURIComponent(canonicalVariantId)}`, {
      ...
      .then((manifest) => {
        ...
        // Use governed canonical media as soon as the full set has passed scanning.
        // Until then, prefer the complete same-origin NOVA source projection so the
        // customer never loses `_2`, `_3`, etc. while the canonical backfill runs.
