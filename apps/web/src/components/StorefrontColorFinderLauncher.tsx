"use client";

import { useEffect, useMemo, useState } from "react";
import styles from "./StorefrontColorFinderLauncher.module.css";

type CategoryOption = Readonly<{ value: string; label: string; count: number }>;

type FinderContext = Readonly<{
  key: "nails";
  title: string;
  categoryCode: string;
  categoryLabel: string;
  route: string;
}>;

const ICON_SRC = "data:image/webp;base64,";

function normalized(value: string): string {
  return value
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .trim()
    .toLocaleLowerCase("el");
}

function isNailCategory(category: CategoryOption): boolean {
  const value = normalized(category.value);
  const label = normalized(category.label);
  return value === "nail-care-colour"
    || value.includes("nail")
    || ["νυχι", "μανικιουρ", "βερνικ"].some((needle) => label.includes(needle));
}

function resolveFinderContext(
  categories: readonly CategoryOption[],
  activeCategory: string,
  activeCategoryGroup: readonly string[],
  vendorId?: string
): FinderContext | null {
  const activeValues = activeCategoryGroup.length
    ? new Set(activeCategoryGroup)
    : activeCategory !== "all"
      ? new Set([activeCategory])
      : null;

  const scopedCategories = activeValues
    ? categories.filter((entry) => activeValues.has(entry.value))
    : categories;

  const nailCategory = scopedCategories.find(isNailCategory);
  if (!nailCategory) return null;

  const params = new URLSearchParams({
    studio: "nails",
    category: nailCategory.value,
    source: "storefront"
  });
  if (vendorId) {
    params.set("vendor", vendorId);
    params.set("returnTo", "/vendor/" + encodeURIComponent(vendorId) + "#products");
  }

  return {
    key: "nails",
    title: "Nail Studio",
    categoryCode: nailCategory.value,
    categoryLabel: nailCategory.label || "Νύχια",
    route: "/color-finder?" + params.toString()
  };
}

export function StorefrontColorFinderLauncher({
  vendorId,
  categories,
  activeCategory,
  activeCategoryGroup,
  colorFacetCount
}: {
  vendorId?: string;
  categories: readonly CategoryOption[];
  activeCategory: string;
  activeCategoryGroup: readonly string[];
  colorFacetCount: number;
}) {
  const [open, setOpen] = useState(false);
  const context = useMemo(
    () => resolveFinderContext(categories, activeCategory, activeCategoryGroup, vendorId),
    [activeCategory, activeCategoryGroup, categories, vendorId]
  );

  useEffect(() => {
    if (!open) return undefined;
    const previousOverflow = document.body.style.overflow;
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === "Escape") setOpen(false);
    };
    document.body.style.overflow = "hidden";
    window.addEventListener("keydown", onKeyDown);
    return () => {
      document.body.style.overflow = previousOverflow;
      window.removeEventListener("keydown", onKeyDown);
    };
  }, [open]);

  if (!context || colorFacetCount < 2) return null;

  return (
    <>
      <button
        className={styles.floatingButton}
        type="button"
        onClick={() => setOpen(true)}
        aria-label={"Άνοιγμα ΚΟΝΤΑ ΜΟΥ Color Finder · " + context.title}
        title={"Color Finder · " + context.title}
      >
        <img src={ICON_SRC} alt="" width={72} height={72} aria-hidden="true" />
        <span className={styles.pulse} aria-hidden="true" />
      </button>

      {open ? (
        <div className={styles.layer} role="presentation">
          <button
            className={styles.backdrop}
            type="button"
            onClick={() => setOpen(false)}
            aria-label="Κλείσιμο Color Finder"
          />
          <section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="storefront-color-finder-title">
            <button className={styles.close} type="button" onClick={() => setOpen(false)} aria-label="Κλείσιμο">×</button>
            <div className={styles.iconWrap}>
              <img src={ICON_SRC} alt="" width={96} height={96} aria-hidden="true" />
            </div>
            <div className={styles.eyebrow}>ΚΟΝΤΑ ΜΟΥ · COLOR FINDER</div>
            <h2 id="storefront-color-finder-title">Βρες πρώτα το χρώμα που θέλεις.</h2>
            <p>
              Στην κατηγορία <strong>{context.categoryLabel}</strong> μπορείς να ψάξεις με βάση την αγαπημένη σου απόχρωση,
              αντί να ανοίγεις ένα-ένα τα προϊόντα.
            </p>
            <div className={styles.info}>
              <span>Τώρα διαθέσιμο</span>
              <strong>{context.title}</strong>
              <small>Διάλεξε χρώμα ή πάρε το από φωτογραφία και δες σχετικές επιλογές με αντιστοίχιση από 49% και πάνω.</small>
            </div>
            <div className={styles.actions}>
              <button className={styles.secondary} type="button" onClick={() => setOpen(false)}>Συνέχισε στο κατάστημα</button>
              <button className={styles.primary} type="button" onClick={() => window.location.assign(context.route)}>
                Άνοιγμα {context.title} →
              </button>
            </div>
          </section>
        </div>
      ) : null}
    </>
  );
}
