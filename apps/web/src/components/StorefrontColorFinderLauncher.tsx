...
function resolveFinderContext(
  ...
  if (activeCategoryGroup.length > 1) return null;
  ...
  return (
    <>
      <button
        ...
        aria-label={"Άνοιγμα ΚΟΝΤΑ ΜΟΥ Color Finder · " + context.presentation.studioLabel}
        title={"Color Finder · " + context.presentation.studioLabel}
      >
      ...
      {open ? (
        <div className={styles.layer} role="presentation">
          ...
          <section className={styles.modal} role="dialog" aria-modal="true" aria-labelledby="storefront-color-finder-title">
            ...
            <div className={styles.eyebrow}>ΚΟΝΤΑ ΜΟΥ · COLOR FINDER · {context.presentation.studioLabel}</div>
            <h2 id="storefront-color-finder-title">{context.presentation.shortcutTitle}</h2>
            <p>
              <strong>{context.category.label}</strong> · {context.presentation.shortcutBody}
            </p>
            <div className={styles.info}>
              <span>Προσαρμοσμένο στην κατηγορία</span>
              <strong>{context.presentation.studioLabel}</strong>
              <small>{context.presentation.photoTitle} Αντιστοιχίζουμε μόνο προϊόντα της τρέχουσας κατηγορίας με αξιόπιστα χρωματικά δεδομένα και όριο 49% και πάνω.</small>
            </div>
