...
export type ExpansionHubStatusRecord = Readonly<{
  id: string;
  slug: string;
  state: ExpansionHubDisplayState;
  prospectCount?: number;
  researchStatus?: "IN_PROGRESS" | "ACTIVE_REFERENCE";
}>;

export type ExpansionHubRuntimeView = ExpansionHub & Readonly<{
  runtimeDisplayState?: ExpansionHubDisplayState;
  runtimeProspectCount?: number;
}>;
...
const ACTIVE_OR_PROSPECT_HUB_STATUS_RECORDS: readonly ExpansionHubStatusRecord[] = [
...
const ACTIVE_OR_PROSPECT_BY_ID = new Map(ACTIVE_OR_PROSPECT_HUB_STATUS_RECORDS.map((record) => [record.id, record] as const));

export const EXPANSION_HUB_STATUS_RECORDS: readonly ExpansionHubStatusRecord[] = EXPANSION_HUBS.map((hub) => {
  const explicit = ACTIVE_OR_PROSPECT_BY_ID.get(hub.id);
  return explicit ?? { id: hub.id, slug: hub.slug, state: "inactive" as const };
});

const HUB_STATUS_BY_ID = new Map(EXPANSION_HUB_STATUS_RECORDS.map((record) => [record.id, record] as const));

export function getExpansionHubDisplayState(hub: ExpansionHub): ExpansionHubDisplayState {
  const runtimeState = (hub as ExpansionHubRuntimeView).runtimeDisplayState;
  return runtimeState ?? HUB_STATUS_BY_ID.get(hub.id)?.state ?? "inactive";
}

export function getExpansionHubProspectCount(hub: ExpansionHub): number | undefined {
  const runtimeCount = (hub as ExpansionHubRuntimeView).runtimeProspectCount;
  return runtimeCount ?? HUB_STATUS_BY_ID.get(hub.id)?.prospectCount;
}
...
export function assertExpansionHubStatusAlignment(): void {
  if (EXPANSION_HUB_STATUS_RECORDS.length !== EXPANSION_HUBS.length) {
    throw new Error(`Expansion hub status snapshot must cover all hubs; found ${EXPANSION_HUB_STATUS_RECORDS.length}`);
  }

  const seen = new Set<string>();
  for (const record of EXPANSION_HUB_STATUS_RECORDS) {
