...
export async function getFastShopTaxonomy(
  ...
  if (!hasFacetContext && !includeDefaultFacets) return fallbackTaxonomy();
  ...
  try {
    const result = await getProductionPostgresRuntime().nativePool.query<FastTaxonomyRow>(`
      WITH RECURSIVE category_tree AS (
        ...
        SELECT category_code AS value,min(category_label) AS label,COUNT(*)::int AS count
        FROM base
      ...
      SELECT
        ...
        COALESCE((
          SELECT jsonb_agg(jsonb_build_object('value',value,'label',label,'count',count) ORDER BY count DESC,label,value)
