...
export type ResearchVendorDossier = {
  ...
  vendor: {
    ...
    researchLocality?: string;
    researchPostcode?: string;
  ...
  return uow.withTransaction(platformScope(principal.userId), async (tx) => {
    const result = await tx.query<SqlRow>(`
      SELECT
        ...
        vrp.distance_km,
        vrp.research_address_line1,
        vrp.research_locality,
        vrp.research_postcode,
        ...
        vrp.online_shop_url,
        vrp.primary_phone,
        vrp.primary_email::text AS primary_email,
    ...
    return {
      ...
      vendor: {
        ...
        censusId: optionalNumber(row.primary_census_id),
        ...
        researchAddress: optionalText(row.research_address_line1),
        researchLocality: optionalText(row.research_locality),
        researchPostcode: optionalText(row.research_postcode),
        ...
        phone: optionalText(row.primary_phone),
        email: optionalText(row.primary_email),
      ...
      sources: sourceResult.rows.map((source): ResearchSourceRecord => ({
        ...
        key: text(source.source_key),
        title: text(source.title),
        checkedAt: optionalText(source.checked_at),
        role: text(source.link_role),
