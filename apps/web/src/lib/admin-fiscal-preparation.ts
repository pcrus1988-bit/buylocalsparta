...
export async function prepareCustomerFiscalDocument(input:PrepareInput):Promise<PreparedResult>{
  ...
  try{
    ...
    const currentPayload=record(d.payload_snapshot);const preparedPayload={...currentPayload,lifecycle:"prepared_for_aade",preparedAt:new Date(now).toISOString(),preparation:{eventCode,accountingPolicyPublicId:p.public_id,policyVersion:p.version,policyHash:p.policy_hash,invoiceType:m.invoice_type,series:s.series,aa,issueDate,payment:{processor,processorMethod,mydataPaymentType:paymentType,transactionId:transactionId??null,tid:paymentTid??null,ecrTokenAttached:Boolean(ecrToken),remoteEcommerce,posInterconnectionExempt:remoteEcommerce,exemptionBasis:remoteEcommerce?"A.1160/2025 remote e-commerce/marketplace":null},lines:preparedLines.map(x=>({lineId:x.lineId,profileId:x.profileId,profileHash:x.profileHash,vatCategory:x.vatCategory,vatRateBps:x.vatRateBps,vatExemptionCategory:x.vatExemptionCategory??null,netMinor:x.netMinor,vatMinor:x.vatMinor,grossMinor:x.grossMinor}))},mydataXml:xml};
