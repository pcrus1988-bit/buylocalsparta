...
async function prepareCustomerFiscalDocument(input:PrepareInput):Promise<PreparedResult>{
  ...
  try{
    ...
    const currentPayload=record(d.payload_snapshot);const preparedPayload={...currentPayload,lifecycle:"prepared_for_aade",preparedAt:new Date(now).toISOString(),preparation:{eventCode,accountingPolicyPublicId:p.public_id,policyVersion:p.version,policyHash:p.policy_hash,invoiceType:m.invoice_type,series:s.series,aa,payment:{processor,processorMethod,mydataPaymentType:paymentType,transactionId:transactionId??null,tid:paymentTid??null,ecrTokenAttached:Boolean(ecrToken)},lines:preparedLines.map(x=>({lineId:x.lineId,profileId:x.profileId,profileHash:x.profileHash,vatCategory:x.vatCategory,vatRateBps:x.vatRateBps,vatExemptionCategory:x.vatExemptionCategory??null,netMinor:x.netMinor,vatMinor:x.vatMinor,grossMinor:x.grossMinor}))},mydataXml:xml};
...
function existingPreparedResult(d:{public_id:string;document_number:string|null;mapping_version:string|null;payload_snapshot:Record<string,unknown>}):PreparedResult{const payload=record(d.payload_snapshot);const prep=record(payload.preparation);const invoiceType=stringValue(prep.invoiceType);const series=stringValue(prep.series);const aa=stringValue(prep.aa);const payment=record(prep.payment);const paymentType=integer(payment.mydataPaymentType);const issueDate=typeof payload.preparedAt==="string"?payload.preparedAt.slice(0,10):"";if(!d.document_number||!d.mapping_version||!invoiceType||!series||!aa||!issueDate)throw new Error("Existing prepared fiscal document is missing preparation metadata");return{ok:true,documentId:d.public_id,documentNumber:d.document_number,invoiceType,series,aa,issueDate,mappingVersion:d.mapping_version,paymentType};}
