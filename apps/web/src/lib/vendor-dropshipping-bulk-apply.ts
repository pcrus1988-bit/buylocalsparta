...
const DEFAULT_SUPPLIER_DEFAULTS_BATCH_SIZE = 300;
const MAX_SUPPLIER_DEFAULTS_BATCH_SIZE = 500;

export type DropshippingSupplierDefaultsBatchResult = Readonly<{
  pricedProducts: number;
  eligibleProducts: number;
  visibleProducts: number;
  overriddenProducts: number;
...
/**
 * Bounded supplier-wide reset used by the vendor Dropshipping UI.
 *
 * The old all-catalogue transaction is intentionally retained above for
 * backward compatibility, but the interactive endpoint must not use it for
 * large supplier catalogues. Each call processes a small UUID-ordered slice
 * and commits before returning a continuation cursor. That keeps the database
 * transaction comfortably below the serverless statement/request timeout.
 */
export async function applyDropshippingSupplierDefaultsBatch(
  vendorIdentity: string,
  supplierCode: string,
  cursor: string | null = null,
  requestedBatchSize = DEFAULT_SUPPLIER_DEFAULTS_BATCH_SIZE
): Promise<DropshippingSupplierDefaultsBatchResult> {
  await assertDropshippingOnlyVendor(vendorIdentity);
  if (!productionDatabaseConfigured()) throw new Error("Η εφαρμογή global Dropshipping ρυθμίσεων απαιτεί ενεργή βάση δεδομένων.");
  ...
  const batchSize = Number.isSafeInteger(requestedBatchSize)
    ? Math.min(MAX_SUPPLIER_DEFAULTS_BATCH_SIZE, Math.max(50, requestedBatchSize))
    : DEFAULT_SUPPLIER_DEFAULTS_BATCH_SIZE;
  ...
  try {
    ...
    const target = await client.query(`
      SELECT dso.id::text supplier_offer_id
       ...
       WHERE dso.supplier_id=$1::uuid
         AND vo.vendor_id=$2::uuid
         AND dso.supplier_cost_minor IS NOT NULL
         AND ($3::uuid IS NULL OR dso.id>$3::uuid)
       ORDER BY dso.id
       LIMIT $4
    `, [supplierId, vendorId, cursor, batchSize + 1]);

    const rows = target.rows.slice(0, batchSize);
    if (!rows.length) {
      await client.query("COMMIT");
      return {
        pricedProducts: 0,
    ...
    const supplierOfferIds = rows.map((row) => String(row.supplier_offer_id));
    const hasMore = target.rows.length > batchSize;
    const nextCursor = hasMore ? supplierOfferIds[supplierOfferIds.length - 1] ?? null : null;
    ...
    const priced = await client.query(`
      ...
      ON CONFLICT(offer_id) DO UPDATE SET
        ...
        updated_at=now()
      RETURNING offer_id
    `, [supplierId, vendorId, defaults.markupPercent, defaults.discountPercent, supplierOfferIds]);
    ...
    if (defaults.visible) {
      ...
      `, [supplierId, vendorId, supplierOfferIds]);
    ...
    const changed = await client.query(`
      UPDATE vendor_offers vo
         SET status=CASE
             ...
             show_msrp=($5::boolean AND vo.msrp_minor IS NOT NULL AND vo.msrp_minor > 0),
             source_payload=(
               COALESCE(vo.source_payload,'{}'::jsonb)
               - 'pricingManualOverride'
               - 'pricingManualOverrideAt'
               - 'pricingFlag'
             ) || jsonb_build_object(
               'pricingPending',false,
               'pricingManagedBy','supplier_defaults_v1',
               'pricingEngine',jsonb_build_object(
                 'version',1,
                 'rule','supplier_markup_discount_v1',
                 'markupPercent',$3::numeric,
                 'discountPercent',$4::numeric,
                 'calculatedAt',now()
               )
             ),
             merchant_visible=CASE
       ...
       WHERE dso.vendor_offer_id=vo.id
         ...
         AND dso.supplier_id=$1::uuid
         AND vo.vendor_id=$2::uuid
         AND dso.id=ANY($7::uuid[])
         AND dso.supplier_cost_minor IS NOT NULL
      RETURNING vo.id,vo.status::text status,vo.merchant_visible,(vo.merchant_visibility_updated_by IS NOT NULL) visibility_overridden
    `, [supplierId, vendorId, defaults.markupPercent, defaults.discountPercent, defaults.showMsrp, defaults.visible, supplierOfferIds]);
