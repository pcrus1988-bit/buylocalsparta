...
/**
 * Apply saved supplier pricing/MSRP defaults using the same sequential rounding
 * as calculateRetailPriceMinor(): round markup first, then round discount on the
 * marked-up amount. Manual per-product visibility decisions are preserved via
 * merchant_visibility_updated_at. Hard supplier/canonical safety gates always
 * win, so an inactive, suppressed or recalled product cannot become public.
 */
