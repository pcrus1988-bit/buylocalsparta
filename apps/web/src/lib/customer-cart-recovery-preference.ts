...
export async function setCustomerCartRecoveryPreference(principal: SessionPrincipal, enabled: boolean, now = Date.now()): Promise<boolean> {
  ...
  const user = await runtime.sqlPool.query<SqlRow>("SELECT id::text AS id FROM users WHERE (public_id=$1 OR id::text=$1) AND status='active' LIMIT 1", [principal.userId]);
