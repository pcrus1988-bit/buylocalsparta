...
export async function getVendorStockFreshness(principal: SessionPrincipal): Promise<VendorStockFreshnessSnapshot> {
  if (!postgresVendorRuntimeEnabled()) return { available: false, freshCount: 0, staleCount: 0, staleSellableCount: 0, items: [] };
  ...
  return uow.withTransaction({ actorUserId: principal.userId, vendorId, marketId: "sparta" }, async (tx) => {
