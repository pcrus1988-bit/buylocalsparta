...
async function readPublicProductSitemapInventory(shard: number | null): Promise<readonly PublicProductSitemapCandidate[]> {
  ...
  const merchantAccountId = text(process.env.GOOGLE_MERCHANT_ACCOUNT_ID);
  ...
  const result = await getProductionPostgresRuntime().nativePool.query<SitemapCandidateRow>(`
    WITH preferred AS MATERIALIZED (
      ...
      WHERE mps.merchant_account_id=$3
      ...
      SELECT lower(BTRIM(mps.last_submitted_payload->'productAttributes'->>'title')) AS title_key,
             GREATEST(1,COUNT(DISTINCT mps.canonical_variant_id))::int AS duplicate_title_count
      FROM public.merchant_product_sync mps
      WHERE mps.merchant_account_id=$3
        AND mps.feed_label='GR'
        AND mps.sync_status='synced'
      ...
      GROUP BY lower(BTRIM(mps.last_submitted_payload->'productAttributes'->>'title'))
    )
    SELECT
      preferred.id,
      cv.slug,
    ...
    JOIN title_counts counts ON counts.title_key=lower(BTRIM(preferred.last_submitted_payload->'productAttributes'->>'title'))
    WHERE cv.active=true
      ...
      AND EXISTS (SELECT 1 FROM public.vendor_offers vo WHERE vo.canonical_variant_id=cv.id AND vo.status='approved' AND vo.merchant_visible=true AND vo.merchant_pause_active=false AND vo.customer_price_minor>0 AND bls_private.vendor_category_effectively_visible(vo.vendor_id,cv.category_id))
    ORDER BY preferred.id
  `, [shard, PRODUCT_SITEMAP_SHARD_COUNT, merchantAccountId]);
  ...
  return result.rows.map((row) => {
    ...
    return {
      ...
      duplicateTitleCount: Number(row.duplicate_title_count) || 1
    };
  });
}
