    ...
    pool.query(`
      SELECT count(*) OVER()::bigint total_count,
             ...
             CASE WHEN c.parent_id IS NULL
                  THEN coalesce(ct_el.name,ct_en.name,c.slug)
                  ELSE coalesce(pct_el.name,pct_en.name,pc.slug)
              END category_name,
       ...
       WHERE ds.id=$1::uuid
         ...
         AND (
           $9::text='all'
           OR ($9='published' AND dso.active AND vo.merchant_visible AND vo.status='approved')
           OR ($9='unpublished' AND NOT (dso.active AND vo.merchant_visible AND vo.status='approved'))
         )
         AND (
           $10::text='all'
         ...
         AND (
           $11::text='all'
           OR ($11='with_cost' AND dso.supplier_cost_minor IS NOT NULL)
           OR ($11='missing_cost' AND dso.supplier_cost_minor IS NULL)
         )
         AND (
           $12::text='all'
           OR ($12='with' AND coalesce(pp.markup_value,0)<>0)
           OR ($12='without' AND coalesce(pp.markup_value,0)=0)
           OR ($12='percent' AND pp.markup_type='percent' AND pp.markup_value IS NOT NULL)
           OR ($12='fixed' AND pp.markup_type='fixed' AND pp.markup_value IS NOT NULL)
         )
         AND (
           $13::text='all'
           OR ($13='with' AND coalesce(pp.discount_value,0)<>0)
           OR ($13='without' AND coalesce(pp.discount_value,0)=0)
           OR ($13='percent' AND pp.discount_type='percent' AND pp.discount_value IS NOT NULL)
           OR ($13='fixed' AND pp.discount_type='fixed' AND pp.discount_value IS NOT NULL)
         )
