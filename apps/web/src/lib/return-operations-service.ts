...
/**
 * Mirrors the core finance model for customer returns:
 * - before payout: reduce the procurement/settlement by the returned vendor proceeds;
 * - after payout: record a vendor receivable for recovery from a later settlement.
 * The operation is audit-idempotent per return so Viva retries cannot double-charge a vendor.
 */
export async function reconcileRefundedReturnFinance(returnId: string, actorUserId: string, now = Date.now()): Promise<void> {
  ...
  await uow.withTransaction({ actorUserId, marketId: "sparta", platformAccess: true }, async (tx) => {
    ...
    for (const row of rows.rows) {
      ...
      // Migration 0041 recalculates payable_minor from merchandise, commission and adjustment_minor.
      // A negative adjustment therefore removes exactly the vendor proceeds allocated to this returned quantity.
