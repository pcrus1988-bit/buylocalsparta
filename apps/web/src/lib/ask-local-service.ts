...
export type AskLocalRequestView = Readonly<{ id: string; status: string; need: string; quantity: number; postcode: string; canonicalVariantId?: string; assignedVendorId?: string; assignedVendorName?: string; responseDueAt?: number; createdAt: number; privateOffers: readonly Readonly<{ id: string; status: string; priceMinor: number; currency: string; fulfilmentPromise?: string; expiresAt: number }>[] }>;
...
export async function submitAskLocal(principal: SessionPrincipal, raw: AskLocalSubmitInput): Promise<AskLocalRequestView> {
  ...
  if (input.canonicalVariantId) {
    ...
    if (!assignment) throw new Error("Το προϊόν δεν έχει αυτή τη στιγμή επιλέξιμο τοπικό συνεργάτη");
  ...
  await uow.withTransaction({ actorUserId: principal.userId, marketId: "sparta", platformAccess: true }, async (tx) => {
    ...
    const vendor = assignedVendorId ? await tx.query<SqlRow>("SELECT id::text AS id,trading_name FROM vendor_businesses WHERE public_id=$1 AND status='active'", [assignedVendorId]) : undefined;
    if (assignedVendorId && !vendor?.rowCount) throw new Error("Selected vendor is not publicly active");
    const offer = assignedVendorId && canonical?.rows[0]?.id ? await tx.query<SqlRow>(`SELECT vo.id::text AS id FROM vendor_offers vo JOIN vendor_locations vl ON vl.id=vo.location_id WHERE vo.vendor_id=$1::uuid AND vo.canonical_variant_id=$2::uuid AND vo.status='approved' AND vl.active=true ORDER BY vo.updated_at DESC,vo.id LIMIT 1`, [vendor!.rows[0].id, canonical.rows[0].id]) : undefined;
    ...
    if (assigned) await tx.query(`INSERT INTO notifications(id,public_id,vendor_id,channel,purpose,event_type,template_version,locale,title,body,payload,status,dedupe_key,created_at)
      VALUES($1,$2,$3,'in_app','transactional','counteroffer.requested','ask-local-v1','el','Νέο ιδιωτικό Ask Local αίτημα',$4,$5::jsonb,'queued',$6,$7) ON CONFLICT(dedupe_key) WHERE dedupe_key IS NOT NULL DO NOTHING`, [randomUUID(), `notification_${randomUUID()}`, vendor!.rows[0].id, input.need.slice(0, 240), JSON.stringify({ requestId: publicId, canonicalVariantId: input.canonicalVariantId, responseDueAt: dueAt?.getTime() }), `ask-local-vendor:${publicId}`, new Date(input.now)]);
...
async function submitMemory(principal: SessionPrincipal, input: ReturnType<typeof validate>): Promise<AskLocalRequestView> {
  let assignedVendorId = input.preferredVendorId;
  if (input.canonicalVariantId) assignedVendorId = (await getCatalogCard(input.canonicalVariantId, `ask-local:${principal.userId}`, input.postcode))?.vendorId;
  const vendor = assignedVendorId ? await getPublicVendorDirectoryEntry(assignedVendorId) : undefined;
  if (assignedVendorId && !vendor) throw new Error("Selected vendor is not publicly active");
  const request: AskLocalRequestView = { id: `cor_${randomUUID()}`, status: vendor ? "awaiting_vendor" : "submitted", need: input.need, quantity: input.quantity, postcode: input.postcode, canonicalVariantId: input.canonicalVariantId, assignedVendorId: vendor?.id, assignedVendorName: vendor?.name, responseDueAt: vendor ? input.now + 24 * 60 * 60 * 1000 : undefined, createdAt: input.now, privateOffers: [] };
