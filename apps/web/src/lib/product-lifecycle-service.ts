...
async function resolveUserUuid(client: any, principal: SessionPrincipal) {
  const result = await client.query(`SELECT id::text AS id FROM public.users WHERE id::text=$1 OR public_id=$1 LIMIT 1`, [principal.userId]);
  const id = result.rows[0]?.id;
  if (typeof id !== "string" || !id) throw new Error("User account could not be resolved");
  return id;
}

async function resolveSubmission(client: any, submissionId: string, lock = false) {
  const result = await client.query(`
    SELECT s.id::text AS submission_uuid,s.public_id,s.vendor_id::text AS vendor_uuid,v.public_id AS vendor_public_id,
           s.canonical_variant_id::text AS canonical_uuid,s.vendor_sku,s.status::text AS submission_status,
    ...
    LEFT JOIN LATERAL (
      SELECT vo.id,vo.public_id,vo.status
...
async function writeAudit(client: any, principal: SessionPrincipal, action: string, entityId: string, reason: string, beforeState: unknown, afterState: unknown) {
  const actor = await resolveUserUuid(client, principal);
  await client.query(`
    INSERT INTO public.audit_events(id,public_id,market_id,actor_user_id,actor_public_id,actor_role,action,entity_type,entity_id,reason,before_state,after_state,created_at)
    VALUES($1,$2,(SELECT id FROM public.markets WHERE code='sparta' LIMIT 1),$3,$4,$5,$6,'vendor_product_submission',$7,$8,$9::jsonb,$10::jsonb,now())
...
export async function adminProductLifecycleState(principal: SessionPrincipal, submissionId: string) {
  ...
  try {
    ...
    const request = row.offer_uuid ? await client.query(`
      SELECT r.public_id,r.status,r.requested_at,r.resolution_note
    ...
    return {
      ...
      activationRequest: request.rows[0] ? {
        id: String(request.rows[0].public_id),
        status: String(request.rows[0].status),
        requestedAt: new Date(request.rows[0].requested_at).getTime(),
        resolutionNote: request.rows[0].resolution_note ? String(request.rows[0].resolution_note) : undefined
      } : undefined
    };
  } finally {
    client.release();
...
export async function archiveAdminProduct(principal: SessionPrincipal, submissionId: string, reason: string) {
  if (reason.trim().length < 3) throw new Error("Archive reason must contain at least 3 characters");
  const client = await pool().connect();
  try {
    await client.query("BEGIN");
    const row = await resolveSubmission(client, submissionId, true);
    const beforeState = { submissionStatus: row.submission_status, offerStatus: row.offer_status };
    ...
    if (row.offer_uuid) await client.query(`UPDATE public.vendor_offers SET status='archived',updated_at=now() WHERE id=$1::uuid`, [row.offer_uuid]);
    if (row.offer_uuid) await client.query(`UPDATE public.vendor_product_activation_requests SET status='cancelled',resolved_at=now(),updated_at=now(),resolution_note='Product archived by admin' WHERE offer_id=$1::uuid AND status='pending'`, [row.offer_uuid]);
    await writeAudit(client, principal, "catalog.product_archived", String(row.public_id), reason.trim(), beforeState, { submissionStatus: "archived", offerStatus: row.offer_uuid ? "archived" : undefined });
...
export async function reactivateAdminProduct(principal: SessionPrincipal, submissionId: string, reason: string) {
  if (reason.trim().length < 3) throw new Error("Activation reason must contain at least 3 characters");
  const client = await pool().connect();
  try {
    await client.query("BEGIN");
    const row = await resolveSubmission(client, submissionId, true);
    if (row.submission_status !== "archived" && row.offer_status !== "archived") throw new Error("Product is not archived");
    const nextSubmissionStatus = row.offer_uuid ? "approved" : row.canonical_uuid ? "linked" : "needs_review";
    ...
    await client.query(`UPDATE public.vendor_product_submissions SET status=$2,updated_at=now() WHERE id=$1::uuid`, [row.submission_uuid, nextSubmissionStatus]);
    if (row.offer_uuid) {
      const actor = await resolveUserUuid(client, principal);
      await client.query(`UPDATE public.vendor_product_activation_requests SET status='approved',resolved_by=$2::uuid,resolved_at=now(),resolution_note=$3,updated_at=now() WHERE offer_id=$1::uuid AND status='pending'`, [row.offer_uuid, actor, reason.trim()]);
    }
    await writeAudit(client, principal, "catalog.product_reactivated", String(row.public_id), reason.trim(), { submissionStatus: row.submission_status, offerStatus: row.offer_status }, { submissionStatus: nextSubmissionStatus, offerStatus: row.offer_uuid ? "approved" : undefined });
    await client.query("COMMIT");
    return { ok: true, status: nextSubmissionStatus };
  } catch (error) {
    await client.query("ROLLBACK");
    throw error;
  } finally {
...
export async function permanentlyDeleteAdminProduct(principal: SessionPrincipal, submissionId: string, reason: string, acknowledged: boolean) {
  ...
  if (reason.trim().length < 3) throw new Error("Deletion reason must contain at least 3 characters");
  const client = await pool().connect();
  try {
    await client.query("BEGIN");
    const row = await resolveSubmission(client, submissionId, true);
    ...
    await writeAudit(client, principal, "catalog.product_deleted", String(row.public_id), reason.trim(), { submissionStatus: row.submission_status, offerStatus: row.offer_status, vendorId: row.vendor_public_id, canonicalVariantId: row.canonical_uuid }, { deleted: true });
...
export async function requestVendorProductActivation(principal: SessionPrincipal, offerId: string) {
  ...
  try {
    ...
    const offer = await client.query(`
      SELECT o.id::text AS offer_uuid,o.public_id,o.vendor_id::text AS vendor_uuid,o.canonical_variant_id::text AS canonical_uuid,o.vendor_sku,o.status::text,
             s.id::text AS submission_uuid,s.public_id AS submission_public_id
      FROM public.vendor_offers o
      JOIN public.vendor_businesses v ON v.id=o.vendor_id
      LEFT JOIN LATERAL (
        SELECT s.id,s.public_id FROM public.vendor_product_submissions s
        ...
        ORDER BY (s.vendor_sku=o.vendor_sku) DESC NULLS LAST,s.updated_at DESC LIMIT 1
      ) s ON true
      WHERE (o.public_id=$1 OR o.id::text=$1) AND (v.public_id=$2 OR v.id::text=$2)
      FOR UPDATE OF o
    `, [offerId, principal.vendorId]);
    if (!offer.rowCount) throw new Error("Product not found in this vendor catalogue");
    const row = offer.rows[0];
    if (String(row.status) !== "archived") throw new Error("Only archived products can request activation");
    const existing = await client.query(`SELECT public_id,status,requested_at FROM public.vendor_product_activation_requests WHERE offer_id=$1::uuid AND status='pending' LIMIT 1`, [row.offer_uuid]);
    ...
    await client.query(`
      INSERT INTO public.vendor_product_activation_requests(id,public_id,vendor_id,offer_id,submission_id,status,requested_by,requested_at,created_at,updated_at)
      VALUES($1,$2,$3::uuid,$4::uuid,$5::uuid,'pending',$6::uuid,now(),now(),now())
    `, [randomUUID(), requestId, row.vendor_uuid, row.offer_uuid, row.submission_uuid ?? null, actor]);
    await client.query("COMMIT");
