...
const plans = [
  {
    ...
    commission: "2%",
    commitment: "36 μήνες",
    bestFor: "Για καταστήματα που θέλουν να μπουν στην πρώτη ομάδα με το χαμηλότερο ποσοστό προμήθειας.",
    note: "Προστατευμένο Founding Partner καθεστώς για 36 μήνες, χωρίς επαναλαμβανόμενη συνδρομή.",
    featured: true
  },
  {
    code: "annual",
    name: "Annual",
    eyebrow: "Καλύτερη ισορροπία κόστους",
    setup: "€299",
    recurring: "€399 / έτος",
    commission: "5%",
    commitment: "Ετήσιο",
    bestFor: "Για ενεργά καταστήματα που θέλουν χαμηλό αρχικό κόστος και χαμηλότερη προμήθεια από το Monthly.",
    note: "Σταθερό ετήσιο πρόγραμμα με πλήρη πρόσβαση στις βασικές λειτουργίες του marketplace.",
    featured: false
  },
  {
    code: "monthly",
    ...
    commission: "7%",
    commitment: "Μηνιαίο",
    bestFor: "Για καταστήματα που προτιμούν μηνιαία συνεργασία και μεγαλύτερη ευελιξία στη διάρκεια.",
  ...
  return <main>
    ...
    <section className="shell page-hero">
      ...
      <div className="hero-actions">
        <a className="button" href="#plans">Σύγκρινε προγράμματα</a>
        <a className="button button-secondary" href="/join/apply?plan=annual">Ξεκίνα αίτηση</a>
        <a className="text-link" href="/join/requirements">Readiness check →</a>
      </div>
    ...
    <section className={`shell section ${styles.planSection}`} id="plans" aria-labelledby="plans-title">
      <div className={styles.planIntro}>
        <div className={styles.planIntroText}>
          <div className="eyebrow">Προγράμματα συνεργασίας</div>
          <h2 id="plans-title">Σύγκρινε πριν κάνεις αίτηση.</h2>
          <p>Και τα τρία προγράμματα δίνουν πρόσβαση στο marketplace και στο vendor workspace. Η βασική διαφορά είναι το κόστος ένταξης, η επαναλαμβανόμενη συνδρομή, η διάρκεια και η προμήθεια πωλήσεων.</p>
        </div>
      ...
      <div className={styles.planGrid}>
        {plans.map((plan) => <article className={`${styles.planCard} ${plan.featured ? styles.planCardFeatured : ""}`} key={plan.code}>
          <span className={styles.planBadge}>{plan.eyebrow}</span>
          <h3 className={styles.planName}>{plan.name}</h3>
          <p className={styles.planBestFor}>{plan.bestFor}</p>
          <div className={styles.planMetrics}>
            <div className={styles.planMetric}><span>Εφάπαξ ένταξη</span><strong>{plan.setup}</strong></div>
            <div className={styles.planMetric}><span>Συνδρομή</span><strong>{plan.recurring}</strong></div>
            <div className={styles.planMetric}><span>Προμήθεια πωλήσεων</span><strong>{plan.commission}</strong></div>
            <div className={styles.planMetric}><span>Διάρκεια / κύκλος</span><strong>{plan.commitment}</strong></div>
          </div>
          <p className={styles.planNote}>{plan.note}</p>
          <a className={`button ${styles.planAction}`} href={`/join/apply?plan=${plan.code}`}>Επίλεξε {plan.name}</a>
        </article>)}
      </div>

      <div className={styles.compareStrip} aria-label="Τι περιλαμβάνουν τα προγράμματα">
        <div><strong>Marketplace παρουσία</strong><span>Merchant profile, προϊόντα και τοπική ανακάλυψη.</span></div>
        <div><strong>Vendor workspace</strong><span>Catalog, stock, παραγγελίες, fulfilment και οικονομική εικόνα.</span></div>
        <div><strong>Fair Vendor Exposure</strong><span>Η ίδια λογική δίκαιης συμμετοχής εφαρμόζεται σε όλα τα ενεργά πλάνα.</span></div>
        <div><strong>Ελεγχόμενη ενεργοποίηση</strong><span>Verification, εμπορικοί όροι και activation gates πριν από οποιαδήποτε χρέωση/ενεργοποίηση.</span></div>
      </div>
