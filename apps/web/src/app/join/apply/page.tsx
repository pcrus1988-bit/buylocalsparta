...
const planOptions = [
  {
    ...
    setup: "€1.500 εφάπαξ",
    recurring: "€0 συνδρομή",
    commission: "2% προμήθεια",
    note: "36 μήνες προστατευμένου Founding Partner καθεστώτος · έως 50 συνεργάτες."
  },
  {
    code: "annual",
    name: "Annual",
    setup: "€299 εφάπαξ",
    recurring: "€399 / έτος",
    commission: "5% προμήθεια",
    note: "Η καλύτερη ισορροπία χαμηλού αρχικού κόστους και χαμηλότερης προμήθειας."
  },
  {
    code: "monthly",
    name: "Monthly",
    setup: "€499 εφάπαξ",
    recurring: "€49 / μήνα",
    commission: "7% προμήθεια",
    note: "Μηνιαία συνεργασία για μεγαλύτερη ευελιξία χωρίς ετήσιο κύκλο."
  }
] as const;
  ...
  return <main>
    ...
    <section className="shell section">
      <div className="section-heading">
        ...
        <p>Διάλεξε ή άλλαξε πρόγραμμα πριν συμπληρώσεις την αίτηση. Η επιλογή αποθηκεύεται μαζί με το application record αλλά δεν δημιουργεί χρέωση ή αυτόματη σύμβαση.</p>
      </div>
      ...
      <div className={styles.applyPlanSelector} id="plan-choice">
        <div className={styles.applyPlanSelectorHeader}>
          <div>
            <div className="eyebrow">Επιλογή προγράμματος</div>
            <h2>Σύγκρινε και διάλεξε πριν υποβάλεις.</h2>
          </div>
          <p>Μπορείς να αλλάξεις επιλογή εδώ ή ξανά μέσα στη φόρμα. Οι τιμές είναι προ ΦΠΑ όπου εφαρμόζεται και επιβεβαιώνονται στους τελικούς εμπορικούς όρους.</p>
        </div>
        <div className={styles.applyPlanGrid}>
          {planOptions.map((plan) => {
            const selected = plan.code === initialPlanCode;
            return <a
              key={plan.code}
              className={`${styles.applyPlanCard} ${selected ? styles.applyPlanCardSelected : ""}`}
              href={`/join/apply?plan=${plan.code}#plan-choice`}
              aria-current={selected ? "true" : undefined}
            >
              <strong>{plan.name}</strong>
              <span>{plan.setup}</span>
              <span>{plan.recurring}</span>
              <span>{plan.commission}</span>
              <small>{plan.note}</small>
              {selected && <span className={styles.selectedLabel}>Επιλεγμένο στην αίτηση</span>}
            </a>;
          })}
        </div>
