...
async function readPublicProductSitemapRouteShard(shard: number): Promise<readonly SitemapRouteCandidate[]> {
  ...
  const result = await getProductionPostgresRuntime().nativePool.query<SitemapRouteRow>(`
    ...
    WHERE rm.canonical_public_id >= $1 AND rm.canonical_public_id < $2 AND rm.eligible_offer_count>0
      AND ((rm.local_sellable=true AND rm.local_available_until>now()) OR (rm.dropship_sellable=true AND rm.dropship_available_until>now()) OR EXISTS (
        SELECT 1 FROM bls_private.storefront_dropship_live_family live
        WHERE live.supplier_id=rm.dropship_supplier_id::uuid AND live.external_product_id=rm.dropship_external_product_id AND live.sellable=true AND live.available_until>now()
      ))
...
const cachedPublicProductSitemapRouteShard = unstable_cache((shard: number) => readPublicProductSitemapRouteShard(shard), ["public-product-sitemap-route-shard-v5"], { revalidate: 900 });
...
export async function GET(_request: Request, { params }: RouteContext): Promise<Response> {
  ...
  if (!settings.indexingEnabled || !settings.sitemap.products) return new Response('<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"></urlset>', { status: 200, headers: { "Content-Type": "application/xml; charset=utf-8" } });
  let products: readonly SitemapRouteCandidate[];
  try { products = await cachedPublicProductSitemapRouteShard(shard); } catch (error) {
