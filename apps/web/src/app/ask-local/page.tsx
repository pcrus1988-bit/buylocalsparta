...
export default async function AskLocalPage({ searchParams }: Props) {
  ...
  return <main>
    ...
    {principal ? <AskLocalClient csrfToken={principal.csrfToken} initial={await customerAskLocalBrowserRequests(principal)} context={context} /> : <section className="shell ask-local-login"><div><div className="eyebrow">Προστασία αιτήματος</div><h2>Συνδέσου για να στείλεις και να παρακολουθείς το αίτημα.</h2><p>Η περιγραφή, η ανάθεση και κάθε ιδιωτική προσφορά παραμένουν στον λογαριασμό σου.</p></div><a className="button" href={`/login?next=${encodeURIComponent(next)}`}>Σύνδεση πελάτη</a></section>}
    <SiteFooter />
  </main>;
}
