...
import { customerPrivateOfferCheckoutView } from "../../../../lib/customer-private-offer-checkout-view";
...
export default async function PrivateOfferCheckoutPage({ params }: Props) {
  ...
  const offer = await customerPrivateOfferCheckoutView(principal, id);
  if (!offer) notFound();
  ...
  return <main>
    ...
    <section className="shell page-hero">
      <div className="eyebrow">Ask Local {offer.requestReference}</div>
      <h1>{offer.existingOrderReference ? "Η προσφορά έγινε ήδη παραγγελία." : "Ολοκλήρωσε την αποδεκτή προσφορά."}</h1>
      <p className="lead">{offer.quantity}× {offer.title} · {money(offer.unitPriceMinor)} / τεμ. · από {offer.vendorName}</p>
    </section>

    <section className="shell page-section">
      {offer.existingOrderReference ? <div className="empty-state"><h2>Υπάρχει ήδη παραγγελία για αυτή την προσφορά.</h2><p>Για να μην δημιουργηθεί διπλή χρέωση, κάθε αποδεκτή ιδιωτική προσφορά μπορεί να συνδεθεί μόνο με μία παραγγελία.</p><Link className="button" href={`/account/orders/${encodeURIComponent(offer.existingOrderReference)}`}>Προβολή παραγγελίας</Link></div>
