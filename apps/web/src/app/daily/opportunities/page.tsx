...
export default async function Page() {
  ...
  return <main className="vendor-app">
    <section className="shell vendor-hero vendor-hero-compact dashboard-hero-refined"><div>
      ...
      <p className="lead">Σήματα ζήτησης σχετικά με τις κατηγορίες του καταστήματός σου, μόνο όταν τουλάχιστον {data.minimumActors} διαφορετικοί actors σχηματίζουν το ίδιο μοτίβο. Δεν εμφανίζονται ονόματα, raw αναζητήσεις, scanned identifiers ή άλλα προσωπικά στοιχεία.</p>
    </div></section>
    ...
    <section className="shell vendor-section">
      ...
      {data.opportunities.length === 0 ? <div className="workspace-empty-state"><strong>Δεν υπάρχει ακόμη αρκετά πυκνό σήμα για το κατάστημά σου.</strong><p>Αυτό είναι σκόπιμο: δεν εμφανίζουμε μικρά clusters που θα μπορούσαν να αποκαλύψουν συμπεριφορά συγκεκριμένου actor.</p><Link className="button button-secondary" href="/daily">Πίσω στο Today</Link></div> : <div className="workspace-queue-list">
        {data.opportunities.slice(0, 20).map((item, index) => <article className="workspace-queue-card" key={item.key}>
          ...
          <div className="workspace-queue-primary"><span>Demand score {item.score}</span><span>{item.signals.distinctActors}+ διαφορετικοί actors</span></div>
          <p className="workspace-queue-summary">Local Watch {item.signals.localWatch} · Ask Local {item.signals.askLocal} · Zero-result {item.signals.zeroResultSearch} · Quick Add miss {item.signals.quickAddMiss} · Saved search {item.signals.savedSearch}</p>
    ...
    <section className="shell vendor-section">
      ...
      <p className="workspace-inline-note">Δεν εμφανίζουμε ποιος έκανε αναζήτηση, postcode, email/τηλέφωνο, raw search query, κείμενο Ask Local, φωτογραφία, voice transcript, barcode, EAN/SKU/model lookup ή Quick Add fingerprint. Zero-result search και Quick Add misses συμμετέχουν μόνο αφού συνδεθούν με product/category context και περάσουν cohort τουλάχιστον {data.minimumActors} διαφορετικών actors.</p>
    </section>
  </main>;
}
