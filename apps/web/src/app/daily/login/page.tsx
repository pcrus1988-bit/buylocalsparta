...
import { DailyLoginForm } from "../../../components/DailyLoginForm";
import { getDailySession } from "../../../lib/daily-session";
import { productionDatabaseConfigured } from "../../../lib/postgres-runtime";

export const metadata: Metadata = { title: "ΚΟΝΤΑ ΜΟΥ Daily · Σύνδεση", robots: { index: false, follow: false } };

export default async function DailyLoginPage() {
  if (await getDailySession()) redirect("/daily");
  ...
  return <main style={{ minHeight: "100dvh", display: "grid", placeItems: "center", padding: 20, background: "#f6f4ee" }}>
    <section style={{ width: "min(100%, 460px)", background: "#fff", border: "1px solid rgba(23,25,20,.1)", borderRadius: 24, padding: "clamp(22px,5vw,38px)", boxShadow: "0 24px 70px rgba(23,25,20,.08)" }}>
      <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: ".14em" }}>ΚΟΝΤΑ ΜΟΥ</div>
      ...
      <p style={{ margin: "0 0 26px", opacity: .66, lineHeight: 1.55 }}>Παραγγελίες, Ask Local, ειδοποιήσεις και παραλαβές QR σε ξεχωριστό λειτουργικό περιβάλλον. Οι κωδικοί Daily δεν παρέχουν πρόσβαση στο πλήρες vendor backoffice.</p>
      {runtimeEnabled ? <DailyLoginForm demoEnabled={demoEnabled} /> : <div className="account-gate"><strong>Η πρόσβαση Daily δεν είναι προσωρινά διαθέσιμη.</strong><p>Επικοινώνησε με την ομάδα ΚΟΝΤΑ ΜΟΥ αν χρειάζεσαι άμεση βοήθεια.</p></div>}
    </section>
  </main>;
}
