...
import { activeFlashCartPricing } from "../../../../lib/flash-sale-entitlement-runtime";
...
async function flashAwareBrowserCart(customerPublicId: string, source: ReturnType<typeof customerBrowserCart>) {
  if (!source?.items.length) return source;
  const pricing = await activeFlashCartPricing(customerPublicId, source.items.map((item) => item.canonicalVariantId));
  if (!pricing.size) return source;
  return {
    ...source,
    items: source.items.map((item) => {
      const flash = pricing.get(item.canonicalVariantId);
      if (!flash) return item;
      return {
        ...item,
        quantity: Math.min(item.quantity, flash.quantityCap),
        priceMinor: flash.flashPriceMinor,
        regularPriceMinor: flash.regularPriceMinor,
        flashSale: true as const,
        quantityCap: flash.quantityCap,
        flashExpiresAt: flash.expiresAt
      };
    })
  };
}
...
export async function GET(request: Request) {
  try {
    ...
    const browserCart = await flashAwareBrowserCart(principal.userId, customerBrowserCart(cart));
    return Response.json({ authenticated: true, persistent: cart !== undefined, csrfToken: cart ? principal.csrfToken : undefined, cart: browserCart });
  } catch (error) {
...
export async function PUT(request: Request) {
  try {
    ...
    const flashPricing = await activeFlashCartPricing(principal.userId, requestedItems.map((item) => item.canonicalVariantId));
    const items = requestedItems.map((item) => {
      const flash = flashPricing.get(item.canonicalVariantId);
      return flash ? { ...item, quantity: Math.min(item.quantity, flash.quantityCap) } : item;
    });

    await syncPersistentCustomerCart(principal, items);
    const cart = await persistentCustomerCart(principal, await getVisitorKey(), postcodeFrom(request));
    const browserCart = await flashAwareBrowserCart(principal.userId, customerBrowserCart(cart));
    return Response.json({ cart: browserCart });
  } catch (error) {
