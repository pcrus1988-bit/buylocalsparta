...
export async function POST(request: Request, context: Context) {
  try {
    ...
    await archiveCustomerNotification({ userId: principal.userId, notificationId, now: Date.now() });
    return Response.json({ ok: true });
  } catch (error) {
    const message = error instanceof Error ? error.message : "notification_archive_failed";
