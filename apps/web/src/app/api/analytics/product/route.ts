...
export async function POST(request: Request) {
  try {
    ...
    const privateHash = privacyVisitorHash(visitorKey);
    ...
    try {
      ...
      if (!assignment.rowCount) {
        ...
        return Response.json({ accepted: false, reason: "no_fairness_assignment" }, { status: 202 });
      }
      ...
      if (eventName) {
        ...
        `, [randomUUID(), `an_${randomUUID()}`, eventName, privateHash, row.selected_vendor_id, row.canonical_variant_id,
