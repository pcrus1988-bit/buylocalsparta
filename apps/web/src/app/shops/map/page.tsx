...
function categoriesFor(vendor: PublicVendorDirectoryEntry): readonly VendorMapCategory[] {
  if (vendor.directoryStatus === "research") {
    return vendor.researchCategory ? [{ key: `research:${categoryKey(vendor.researchCategory)}`, label: vendor.researchCategory }] : [];
  }
  ...
  return (
    <main>
      ...
      <section className="page-hero">
        <div className="shell">
          ...
          <div className="hero-proof">
            <span><strong>{mappedCount}</strong> επιχειρήσεις με διαθέσιμο σημείο</span>
      ...
      <section className="shell section" id="vendor-map" aria-labelledby="vendor-map-title">
        ...
        {vendors.length ? <VendorMapDirectory vendors={vendors} categories={categories} /> : <div className="empty-state"><h2>Η βάση καταστημάτων ετοιμάζεται.</h2><p>Δεν υπάρχουν ακόμη δημόσιες καταχωρίσεις στην παραγωγική βάση δεδομένων.</p><a className="button" href="/shops">Πίσω στα καταστήματα</a></div>}
      </section>
      <SiteFooter />
    </main>
  );
