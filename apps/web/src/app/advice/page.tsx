...
export default async function AdvicePage({ searchParams }: Props) {
  ...
  return <main>
    <div className="announcement">Πραγματική συμβουλή · πριν την αγορά, μίλα με άνθρωπο που γνωρίζει.</div><SiteHeader />
    <section className="advice-hub-hero"><div className="shell advice-hub-hero-grid"><div><div className="eyebrow light">Ανθρώπινο τοπικό εμπόριο</div><h1>{product ? `Συμβουλή για: ${product.title}` : requestedVendor ? `Μίλα με το ${requestedVendor.name}` : "Η ψηφιακή αγορά ξαναγίνεται ανθρώπινη."}</h1><p>{requestedVendor?.adviser ? `Ο/Η ${requestedVendor.adviser} μπορεί να σε βοηθήσει μέσα από το δημόσιο προφίλ του καταστήματος.` : "Πες μας τι ψάχνεις και το Ask Local θα δρομολογήσει το αίτημά σου στον κατάλληλο ενεργό επαγγελματία, χωρίς δημόσια διαδικασία προσφορών."}</p><div className="hero-actions">{requestedVendor ? <><a className="button button-light" href={`/account/appointments?vendor=${encodeURIComponent(requestedVendor.id)}`}>Κλείσε ραντεβού</a><a className="button vendor-outline" href={`/vendor/${requestedVendor.id}`}>Προφίλ καταστήματος</a></> : <a className="button button-light" href="/ask-local">Ρώτησε την τοπική αγορά</a>}<a className="button vendor-outline" href="#advisers">Δες συμβούλους</a></div></div><div className="advice-hub-signal" aria-hidden="true"><span>ΡΩΤΗΣΕ</span><span>ΑΚΟΥΣΕ</span><span>ΕΠΙΛΕΞΕ</span></div></div></section>
    <section className="shell advice-path" aria-label="Πώς λειτουργεί η συμβουλή"><article><span>01</span><h2>Πες τι χρειάζεσαι</h2><p>Ξεκίνα από προϊόν, κατηγορία ή συγκεκριμένο κατάστημα.</p></article><article><span>02</span><h2>Ιδιωτική καθοδήγηση</h2><p>Η ερώτηση απευθύνεται στον κατάλληλο επαγγελματία, όχι σε δημόσια διαδικασία προσφορών.</p></article><article><span>03</span><h2>Αγόρασε όταν είσαι έτοιμος</h2><p>Η αγορά παραμένει στην ενιαία διαδικασία ολοκλήρωσης αγοράς του ΚΟΝΤΑ ΜΟΥ Σπάρτη.</p></article></section>
