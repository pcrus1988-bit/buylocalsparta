...
export default async function VendorPage({ params }: Props) {
  const { id } = await params;
  const vendor = await getPublicVendorDirectoryEntry(id);
  if (!vendor || vendor.directoryStatus !== "partner") notFound();
    ...
    <main>
      ...
      <section className="vendor-profile-hero">
        <div className="shell vendor-profile-grid">
          <div>
            ...
            <div className="eyebrow light">{location?.locality ? `Συνεργαζόμενο κατάστημα · ${location.locality}` : "Συνεργαζόμενο τοπικό κατάστημα"}</div>
            <h1>{vendor.name}</h1>
            ...
            <div className="vendor-profile-actions"><a className="button button-light" href={`/ask-local?vendor=${encodeURIComponent(vendor.id)}`}>Ρώτησε {vendor.adviser ?? "το κατάστημα"}</a><a className="button vendor-outline" href="/shop">Δες την αγορά</a></div>
          </div>
      ...
      <section className="shell vendor-profile-summary" aria-label="Στοιχεία καταστήματος">
        <div className="vendor-profile-facts">
          ...
          {location?.phone && <div className="vendor-fact"><strong>Τηλέφωνο</strong><span><a href={`tel:${location.phone}`}>{location.phone}</a></span></div>}{location?.publicEmail && <div className="vendor-fact"><strong>Email</strong><span><a href={`mailto:${location.publicEmail}`}>{location.publicEmail}</a></span></div>}{categories.length > 0 && <div className="vendor-fact"><strong>Κατηγορίες</strong><div className="vendor-category-row">{categories.map((category) => <span className="shop-category-chip" key={category.slug}>{category.label}</span>)}</div></div>}
        </div>
        <article className="vendor-story-card"><div className="eyebrow">Η ιστορία του καταστήματος</div>{vendor.story ? <><h2>{vendor.story.title}</h2><p>{vendor.story.excerpt}</p><small className="vendor-story-approval">Δημοσιευμένο merchant story με καταγεγραμμένη έγκριση του vendor{storyMedia ? " και εγκεκριμένο οπτικό υλικό." : "."}</small></> : <><h2>Πραγματικός άνθρωπος, όχι απρόσωπη καταχώριση.</h2><p>Δεν έχει δημοσιευθεί ακόμη εγκεκριμένη ιστορία για αυτό το κατάστημα. Η πλατφόρμα δεν συμπληρώνει δημόσιο storytelling χωρίς έγκριση.</p></>}</article>
      </section>
      <section className="shell section"><div className="section-heading"><div><div className="eyebrow">Η επιλογή του συνεργάτη</div><h2>Προϊόντα που εξυπηρετούνται εδώ τώρα</h2></div><p className="section-note">Η σελίδα δείχνει canonical προϊόντα που το κατάστημα μπορεί να εξυπηρετήσει, χωρίς να δημιουργεί Fair Vendor Exposure γεγονότα ή να αποκαλύπτει κρυφές τιμές προμηθευτή.</p></div>{products.length ? <div className="product-grid">{products.map((product, index) => <CatalogProductCard product={product} index={index} vendorContext={{ name: vendor.name, adviser: vendor.adviser }} key={product.id} />)}</div> : <div className="empty-state"><h2>Δεν υπάρχουν διαθέσιμα canonical προϊόντα για αυτό το προφίλ.</h2><p>Το κατάστημα μπορεί να παραμένει δημόσια ορατό χωρίς να εμφανίζονται ανενεργές ή μη διαθέσιμες προσφορές προϊόντων.</p><a className="button" href="/shop">Δες όλα τα προϊόντα</a></div>}</section>
      <SiteFooter />
    </main>
  );
}
