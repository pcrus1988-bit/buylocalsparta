  ...
  return <main className="vendor-app admin-app admin-catalogue-overview">
      ...
      items={[
        {
          ...
          hint: `${data.metrics.liveProducts} active canonical`
        },
        {
          label: "Taxonomy",
          value: data.metrics.totalCategories,
          hint: `${data.metrics.activeCategories} active nodes`
        },
        {
          label: "Semantic coverage",
          value: `${data.attributes.semanticCoveragePct}%`,
          tone: data.attributes.unmappedObservations > 0 ? "attention" : "positive",
          hint: `${data.attributes.unmappedObservations.toLocaleString("el-GR")} unmapped observations`
        },
        {
          label: "Empty branches",
          value: data.metrics.emptyCategories,
          tone: data.metrics.emptyCategories > 0 ? "attention" : "positive",
          hint: "No products in the whole branch"
        }
      ]}
    />
    ...
    <section className="shell vendor-section">
      ...
      <div className="catalogue-workflow-grid">
        <Link className="catalogue-workflow-card" href="/admin/quickadd">
          <span>Acquire · one product</span>
          <strong>Quick Add</strong>
          <p>Barcode/search → reuse canonical or create safely → assign vendor offer and stock.</p>
          <i>Open workbench →</i>
        </Link>
        <Link className="catalogue-workflow-card" href="/admin/catalogue-crawler">
          <span>Acquire · website</span>
          <strong>Website Import</strong>
          <p>Crawl one page or a full online shop and promote extracted evidence into Supplier PIM.</p>
          <i>Open website import →</i>
        </Link>
        <Link className="catalogue-workflow-card" href="/admin/catalogue-intake/import">
          <span>Acquire · files & providers</span>
          <strong>Files & Icecat</strong>
          <p>Supplier CSV/TSV, trusted adapters and Open Icecat staging/enrichment status.</p>
          <i>Open source import →</i>
        </Link>
        <Link className="catalogue-workflow-card" href="/admin/catalogue-intake">
          <span>Normalize · source evidence</span>
          <strong>Supplier PIM</strong>
          <p>Immutable snapshots, price/classification review, source evidence and vendor assortment hand-off.</p>
          <i>Open PIM review →</i>
        </Link>
        <Link className={`catalogue-workflow-card${data.attributes.unmappedObservations > 0 ? " needs-attention" : ""}`} href="/admin/catalogue-intake/attributes">
          <span>Normalize · attributes</span>
          <strong>Attributes</strong>
          <p>Resolve repeated supplier attribute meanings once per governed source context.</p>
          <b>{data.attributes.unmappedObservations.toLocaleString("el-GR")}</b>
          <i>unmapped observations →</i>
        </Link>
        <Link className="catalogue-workflow-card" href="/admin/matching">
          <span>Resolve identity</span>
          <strong>Product Matching</strong>
          <p>Approve canonical candidates, reject false matches or create a genuinely new canonical.</p>
          <i>Open matching queue →</i>
        </Link>
        <Link className={`catalogue-workflow-card${data.metrics.emptyCategories > 0 ? " needs-attention" : ""}`} href="/admin/categories">
          <span>Govern structure</span>
          <strong>Categories & Policies</strong>
          <p>Commerce policies and category-level operating rules. Taxonomy health remains visible below.</p>
          <b>{data.metrics.emptyCategories}</b>
          <i>empty branches →</i>
        </Link>
      </div>
    </section>
    ...
    <section className="vendor-section section-tint">
      <div className="shell">
        <WorkspaceSectionHeading
          eyebrow="Attention"
          title="Τι χρειάζεται δουλειά τώρα"
          note="Health signals only. The actual decisions stay in their dedicated operational queues."
        />
        <div className="catalogue-attention-grid">
          <article className="workspace-queue-card">
            <div className="workspace-queue-head">
              <div>
                <strong>Supplier attribute normalization</strong>
                <small>Raw supplier evidence remains preserved; this measures semantic mapping only.</small>
              </div>
              <WorkspaceStatusBadge
            ...
            {data.unmappedAttributes.length > 0 && <WorkspaceRecordDetails label={`Top unresolved contexts · ${data.unmappedAttributes.length}`}>
            ...
            <div className="workspace-action-bar">
              <span>Resolve repeated source keys once in their governed context.</span>
              <Link className="button button-secondary" href="/admin/catalogue-intake/attributes">Open Attributes</Link>
            </div>
          </article>
          ...
          <article className="workspace-queue-card">
            <div className="workspace-queue-head">
              <div>
                <strong>Taxonomy health</strong>
                <small>Structure, assignability and product coverage without turning the overview into a second policy editor.</small>
              </div>
              <WorkspaceStatusBadge
                status={data.metrics.emptyCategories > 0 ? "attention" : "active"}
                label={data.metrics.emptyCategories > 0 ? `${data.metrics.emptyCategories} empty branches` : "Healthy"}
              />
            </div>
            <div className="workspace-queue-primary">
            ...
            <div className="workspace-action-bar">
              <span>Commerce rules are owned by Categories & Policies.</span>
              <Link className="button button-secondary" href="/admin/categories">Open Categories</Link>
            </div>
          </article>
        </div>
      </div>
    ...
    <section className="shell vendor-section">
      <WorkspaceSectionHeading
        eyebrow="Catalogue model"
        title="Product Types, attributes & taxonomy structure"
        note="Reference information stays available here, but detailed review and policy changes belong to the dedicated workspaces above."
      />
      <WorkspaceMetricStrip
      ...
      {data.categories.length === 0 ? <WorkspaceEmptyState
      ...
      /> : <WorkspaceRecordDetails label={`Browse taxonomy tree · ${data.categories.length} nodes`}>
        <div className="catalogue-taxonomy-list">
          {data.categories.map((category) => <details className={`catalogue-taxonomy-row depth-${Math.min(category.depth, 5)}`} key={category.categoryCode}>
            <summary>
              <span className="catalogue-taxonomy-identity">
