...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="vendor-app admin-app">
    ...
    <WorkspaceMetricStrip items={[
      ...
      { label: "Agreement", value: shop.cooperationDocumented ? "documented" : "attention", tone: shop.cooperationDocumented ? "positive" : "attention" },
    ...
    <section className="shell vendor-section admin-anchor-section">
      <WorkspaceSectionHeading eyebrow="Overview" title="Operational state & public visibility" note="Lifecycle gates remain unchanged. This page only centralises the controls and evidence around one partner." />
      ...
      {!shop.operationalActive && !shop.cooperationDocumented && <div className="workspace-inline-note">Operational activation is blocked until the cooperation agreement is fully documented.</div>}
    ...
    <section id="partner-agreement" className="shell vendor-section admin-anchor-section"><WorkspaceSectionHeading eyebrow="Agreement & SLA" title="Commercial agreement & service levels" note="Commercial terms and SLA belong to the partner record even though the underlying governed endpoints remain unchanged." />{shop.agreement ? <div className="workspace-compact-list"><div className="workspace-compact-row"><strong>Agreement</strong><span>{shop.agreement.code} · v{shop.agreement.version} · {shop.agreement.status}</span><small>{shop.agreement.id}</small></div><div className="workspace-compact-row"><strong>Signed / document</strong><span>{shop.agreement.signedAt ? new Date(shop.agreement.signedAt).toLocaleDateString("el-GR") : "Not signed"} · {shop.agreement.sourceDocumentReference ?? "No document reference"}</span></div><div className="workspace-compact-row"><strong>Commercial terms</strong><span>{(shop.agreement.commissionRateBps / 100).toLocaleString("el-GR")} % commission · listing {euro(shop.agreement.listingFeeMinor)} · recurring {euro(shop.agreement.recurringFeeMinor)} {shop.agreement.recurringFeePeriod ?? ""}</span></div>{slaPolicy && <div className="workspace-compact-row"><strong>Order SLA</strong><span>{slaPolicy.acceptanceMinutes} min acceptance · {slaPolicy.preparationMinutes} min preparation</span><small>{slaPolicy.configured ? `Configured · warning ${slaPolicy.warningPercent}% · email ${slaPolicy.emailReminderPercent}%` : "Fallback policy"}</small></div>}</div> : <div className="workspace-inline-note">Δεν υπάρχει cooperation agreement record για αυτό το partner.</div>}<WorkspaceRecordDetails label="Contract workflow"><VendorAgreementForm vendorId={shop.id} csrfToken={managed.csrfToken} defaults={shop.agreement ? { code: shop.agreement.code, commissionRateBps: shop.agreement.commissionRateBps, listingFeeMinor: shop.agreement.listingFeeMinor, recurringFeeMinor: shop.agreement.recurringFeeMinor, recurringFeePeriod: shop.agreement.recurringFeePeriod, sourceDocumentReference: shop.agreement.sourceDocumentReference } : undefined} /></WorkspaceRecordDetails>{canFinance && <div className="workspace-action-bar"><span>SLA policies are configured per agreement.</span><div className="workspace-action-buttons"><Link className="button button-secondary" href="/admin/finance/agreements">Agreements</Link><Link className="button button-secondary" href="/admin/finance/agreements/sla">SLA policies</Link></div></div>}</section>
