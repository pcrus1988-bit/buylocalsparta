        ...
        : <div className="workspace-queue-list">
          {data.rows.map((row) => {
            ...
            return <article className="workspace-queue-card" key={row.id}>
              ...
              <div className="workspace-action-bar"><span>Last registry sync: {new Intl.DateTimeFormat("el-GR", { dateStyle: "short", timeStyle: "short", timeZone: "Europe/Athens" }).format(new Date(row.lastSeenAt))}</span><div className="workspace-action-buttons"><Link className="text-link" href={row.route} target="_blank">Open public page ↗</Link>{row.openIssues > 0 && <Link className="text-link" href="/admin/seo/issues">Review issues →</Link>}</div></div>
            </article>;
          })}
        </div>}
