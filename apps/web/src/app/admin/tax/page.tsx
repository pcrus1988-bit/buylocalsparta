  ...
  return <main className="vendor-app admin-app">
    ...
    <section className="shell vendor-section">
      ...
      {data.documents.length === 0 ? <WorkspaceEmptyState title="Δεν υπάρχουν tax documents ακόμη." body="A captured payment can create a pending fiscal record automatically when enabled in the Admin configuration, or an Admin can recover one manually by order ID above." /> : <div className="workspace-queue-list">{data.documents.map(document => <article className="workspace-queue-card" key={document.id}>
        ...
        {document.transmissionStatus === "manual_review" && document.documentNumber && !document.aadeMark && <div className="workspace-action-bar"><span>Read-only recovery only: search AADE for this exact numbered document. This action never resends SendInvoices.</span><div className="workspace-action-buttons"><AdminActionButton label="Reconcile with AADE" endpoint="/api/admin/tax/reconcile" csrfToken={principal.csrfToken} body={{documentId:document.id}} reasonPrompt="Αιτιολογία read-only AADE reconciliation" /></div></div>}
