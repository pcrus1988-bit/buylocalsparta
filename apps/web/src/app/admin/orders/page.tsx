...
export default async function Page({ searchParams }: { searchParams: Promise<{ customer?: string; order?: string }> }) {
  ...
  const openOrders = orders.filter((order) => !["cancelled", "completed", "fulfilled", "refunded"].includes(order.status)).length;
  ...
  return <main className="vendor-app admin-app">
    ...
    <section className="shell vendor-hero vendor-hero-compact dashboard-hero-refined">
      <div><div className="eyebrow">Customer operations</div><h1>Orders & returns</h1><p className="lead">Παρακολούθησε εξαιρέσεις και επόμενες ενέργειες χωρίς να χάνεται η μία ενιαία customer order.</p>{filtered && <div className="hero-actions"><span className="status-pill">Filtered{customerFilter ? ` · customer ${customerFilter}` : ""}{orderFilter ? ` · order ${orderFilter}` : ""}</span><Link className="text-link" href="/admin/orders">Clear filter →</Link>{customerFilter && <Link className="text-link" href={`/admin/customers/${encodeURIComponent(customerFilter)}`}>Customer 360 →</Link>}</div>}</div>
    </section>
    ...
    <section className="shell vendor-section">
      ...
      {orders.length === 0 ? <WorkspaceEmptyState title={filtered ? "Δεν βρέθηκαν παραγγελίες για αυτό το customer/order filter." : "Δεν υπάρχουν ακόμη παραγγελίες."} /> : <div className="workspace-queue-list">{orders.map((order) => <article className="workspace-queue-card" key={order.id}>
        ...
        <div className="workspace-action-bar"><span>Order status: <strong>{order.status}</strong></span><div className="workspace-action-buttons">{order.customerId && <Link className="button button-secondary" href={`/admin/customers/${encodeURIComponent(order.customerId)}`}>Customer 360</Link>}{!["cancelled", "completed", "fulfilled", "refunded"].includes(order.status) && <AdminActionButton label="Cancel order" endpoint="/api/admin/orders/action" csrfToken={data.csrfToken} body={{ orderId: order.id, action: "cancel" }} reasonPrompt="Platform cancellation reason" danger />}</div></div>
      </article>)}</div>}
    </section>
