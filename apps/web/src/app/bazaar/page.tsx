...
import { bazaarConditionLabel, bazaarDisplayConditionLabel, bazaarProductDisclosure, bazaarProductNoticeSummary, bazaarProductNoticeTitle, bazaarSourceLabel, getBazaarCatalog } from "../../lib/bazaar-catalog";
...
export async function generateMetadata({ searchParams }: BazaarPageProps): Promise<Metadata> {
  const base = await governedStaticSeoMetadata("/bazaar", {
    title: "BAZAAR",
    description: "Preloved, Preowned / Defect, open-box, tester, sample και επιλεγμένα επιστρεφόμενα προϊόντα από όλη την Ελλάδα, σε ξεχωριστό BAZAAR του KONTA MOY."
  });
    ...
    <section className="shell" style={{ paddingTop: 56, paddingBottom: 32 }}>
      <div style={{ display: "grid", gap: 18, maxWidth: 900 }}>
        ...
        <p className="lead" style={{ maxWidth: 760, fontSize: "clamp(1.05rem, 2vw, 1.35rem)" }}>
          Preloved, Preowned / Defect, open-box, tester, sample και επιλεγμένες επιστροφές. Κάθε προϊόν είναι ξεχωριστά ταξινομημένο και δεν εμφανίζεται ποτέ στον κανονικό κατάλογο του KONTA MOY.
        </p>
          ...
          return <article className="product-card" key={product.id} style={{ overflow: "hidden", position: "relative" }}>
            ...
            <div className="product-body">
              ...
              {bazaarProductDisclosure(product.bazaarSource) ? <details style={{ margin: "0 0 10px", fontSize: ".78rem" }}>
                <summary style={{ cursor: "pointer", fontWeight: 800 }}>{bazaarProductNoticeTitle(product.bazaarSource)} · Περισσότερα</summary>
                <p style={{ margin: "6px 0 0", lineHeight: 1.35 }}>{bazaarProductNoticeSummary(product.bazaarSource)}</p>
              </details> : null}
              <div className="price">
