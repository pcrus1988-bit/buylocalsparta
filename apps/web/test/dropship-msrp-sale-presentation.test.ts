...
const pricePresentationUrl = new URL("../src/lib/public-price-presentation.ts", import.meta.url);
...
test("MSRP savings math follows the BAZAAR convention", async () => {
  const source = await readFile(pricePresentationUrl, "utf8");

  assert.match(source, /\(\(msrpMinor - retailPriceMinor\) \/ msrpMinor\) \* 1000/);
  assert.match(source, /msrpMinor <= retailPriceMinor/);
  assert.match(source, /kind === "sale" \? `SALE · −\$\{savingLabel\}%` : `−\$\{savingLabel\}%`/);
});
