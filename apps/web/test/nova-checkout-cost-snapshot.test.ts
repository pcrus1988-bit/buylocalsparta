...
test("Nova checkout snapshots the live revalidated supplier buying cost", async () => {
  ...
  assert.match(source, /supplierCostMinor = evidence\\.supplierCostMinor/);
  assert.match(source, /validated\\.push\\(\\{ row, quantity: item\\.quantity, supplierCostMinor, revalidatedAt \\}\\)/);
