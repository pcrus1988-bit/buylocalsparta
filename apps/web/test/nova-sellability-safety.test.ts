...
test("Nova sellability sweep deactivates explicit supplier unavailability, not TTL expiry", async () => {
  ...
  assert.doesNotMatch(source, /supplier_availability_expired/);
  assert.doesNotMatch(source, /availability_expires_at IS NOT NULL AND dso\.availability_expires_at <= now\(\)/);
});
