...
CREATE TEMP TABLE taxonomy_0129_definitions (
...
INSERT INTO taxonomy_0129_definitions (
...
BEGIN
  SELECT count(*) INTO v_definition_count FROM taxonomy_0129_definitions;
  IF v_definition_count <> 13 THEN
    RAISE EXCEPTION '0129 taxonomy definition assertion failed: expected 13 definitions, got %', v_definition_count;
  END IF;

  SELECT count(*) INTO v_market_count FROM public.markets;
  IF v_market_count = 0 THEN
    RAISE EXCEPTION '0129 taxonomy expansion requires at least one market';
  END IF;

  SELECT count(*) INTO v_resolved_count
  FROM public.markets m
  CROSS JOIN taxonomy_0129_definitions d
  ...
  IF v_resolved_count <> v_definition_count * v_market_count THEN
    RAISE EXCEPTION '0129 taxonomy dependency assertion failed: expected % resolved market/definition pairs, got %', v_definition_count*v_market_count, v_resolved_count;
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '0129 taxonomy slug collision found for an existing category';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '0129 taxonomy code collision found for an existing category';
  END IF;
  ...
  FROM public.markets m CROSS JOIN taxonomy_0129_definitions d
  ...
  IF v_category_count <> v_definition_count*v_market_count
    ...
    RAISE EXCEPTION '0129 taxonomy final assertion failed: categories %, translations %, type links %', v_category_count,v_translation_count,v_type_link_count;
  END IF;

  RAISE NOTICE '0129 machinery/equipment taxonomy ready: % product classes across % market(s)', v_definition_count,v_market_count;
END
$$;
