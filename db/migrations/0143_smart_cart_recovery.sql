...
CREATE INDEX cart_recovery_attempts_user_time_idx
  ON public.cart_recovery_attempts(user_id, created_at DESC);
...
BEFORE UPDATE OR DELETE ON public.cart_recovery_attempts
FOR EACH ROW EXECUTE FUNCTION public.prevent_cart_recovery_attempt_mutation();
...
CREATE POLICY cart_recovery_attempts_privacy_delete
  ON public.cart_recovery_attempts FOR DELETE
  USING (
