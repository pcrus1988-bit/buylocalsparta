...
CREATE TEMP TABLE taxonomy_0204_definitions (
...
INSERT INTO taxonomy_0204_definitions (
...
BEGIN
  SELECT count(*) INTO v_definition_count FROM taxonomy_0204_definitions;
  IF v_definition_count <> 12 THEN
    RAISE EXCEPTION '0204 taxonomy definition assertion failed: expected 12 definitions, got %', v_definition_count;
  END IF;

  SELECT count(*) INTO v_market_count FROM public.markets;
  IF v_market_count = 0 THEN
    RAISE EXCEPTION '0204 taxonomy expansion requires at least one market';
  END IF;

  SELECT count(*) INTO v_resolved_count
  FROM public.markets m
  CROSS JOIN taxonomy_0204_definitions d
  ...
  IF v_resolved_count <> v_definition_count * v_market_count THEN
    RAISE EXCEPTION '0204 taxonomy dependency assertion failed: expected % resolved market/definition pairs, got %',
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '0204 taxonomy slug collision found for an existing category';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '0204 taxonomy code collision found for an existing category';
  END IF;
  ...
  IF v_category_count <> v_definition_count*v_market_count
    ...
    RAISE EXCEPTION '0204 taxonomy final assertion failed: categories %, translations %, type links %',
