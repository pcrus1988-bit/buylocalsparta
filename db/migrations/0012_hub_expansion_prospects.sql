...
CREATE TABLE IF NOT EXISTS hub_expansion_prospects (
  ...
  address_line text NOT NULL CHECK (length(btrim(address_line)) BETWEEN 3 AND 180),
...
CREATE UNIQUE INDEX IF NOT EXISTS hub_expansion_prospects_open_business_uidx
  ON hub_expansion_prospects(hub_slug, lower(email), lower(business_name))
...
COMMENT ON TABLE hub_expansion_prospects IS
  'Pre-launch merchant interest for non-Sparta expansion HUBs. Prospect records never imply active marketplace/vendor access.';
