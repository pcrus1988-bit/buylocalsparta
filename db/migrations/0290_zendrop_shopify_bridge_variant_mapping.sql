-- Add a private, durable mapping between Zendrop variants and the hidden Shopify bridge.
-- This migration is idempotent because the physical table was commissioned through Supabase first.
  ...
  IF NOT EXISTS (
    SELECT 1
     ...
     WHERE schemaname='public'
       AND tablename='dropship_shopify_bridge_variants'
       AND policyname='bls_platform_runtime_all'
  ) THEN
...
GRANT SELECT,INSERT,UPDATE,DELETE
  ON public.dropship_shopify_bridge_variants
...
COMMENT ON COLUMN public.dropship_shopify_bridge_variants.metadata IS
  'Non-secret synchronization evidence only. Credentials remain in server-side environment variables.';
