...
ALTER TABLE public.canonical_variants
  ...
  CHECK (condition = ANY (ARRAY['new'::text,'refurbished'::text,'used'::text,'preloved'::text,'preowned_defect'::text,'open_box'::text]));
...
ALTER TABLE public.canonical_variants
  ...
  CHECK (bazaar_source IS NULL OR bazaar_source = ANY (ARRAY['supplier_preloved'::text,'supplier_preowned_defect'::text,'customer_return'::text,'open_box'::text,'display_stock'::text,'damaged_packaging'::text,'admin_curated'::text]));
...
ALTER TABLE public.canonical_variants
  ...
  CHECK (condition NOT IN ('preloved','preowned_defect','open_box') OR commerce_channel = 'bazaar');

ALTER TABLE public.canonical_variants DROP CONSTRAINT IF EXISTS canonical_variants_market_id_slug_key;
DROP INDEX IF EXISTS public.canonical_variants_market_id_slug_key;
CREATE UNIQUE INDEX canonical_variants_market_id_slug_key ON public.canonical_variants (market_id,commerce_channel,slug);

DROP INDEX IF EXISTS public.canonical_variants_gtin_unique;
CREATE UNIQUE INDEX canonical_variants_gtin_unique ON public.canonical_variants (market_id,gtin)
...
COMMENT ON COLUMN public.canonical_variants.bazaar_source IS
  'Optional provenance for BAZAAR inventory, including supplier preloved/defect, returns, open-box, display stock and damaged packaging.';
