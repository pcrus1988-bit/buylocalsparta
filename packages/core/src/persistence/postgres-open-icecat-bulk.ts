    ...
    return this.#uow.withTransaction({ platformAccess: true }, async (tx) => {
      ...
      const run = requireSingleRow(await tx.query<SqlRow>(`
        INSERT INTO public.open_icecat_bulk_ingestion_runs (
          source_id, import_kind, source_url, source_fingerprint, status
        ) VALUES ($1::uuid,$2,$3,$4,'running')
        ON CONFLICT (source_id,import_kind,source_fingerprint) DO UPDATE
        SET source_url=EXCLUDED.source_url,
            ...
            END,
            updated_at=now()
        RETURNING id::text, checkpoint, status, persisted, removed, rejected, filtered
      `, [source.id, identity.importKind, identity.sourceUrl, identity.sourceFingerprint]));
    ...
    return this.#uow.withTransaction({ platformAccess: true }, async (tx) => {
      const result = await tx.query<SqlRow>(`
        ...
        SELECT r.id::text AS run_id, r.source_id::text AS source_id,
               ...
               r.import_kind, r.source_url, r.source_fingerprint, r.status,
