...
import { normalizeNovaProduct, novaAvailability } from "../../../integrations/dropship-suppliers/src/nova-normalize.ts";

test("Nova normalization preserves source identity and keeps price semantics staged", () => {
  ...
  assert.equal(evidence.qualityPayload.pricingSemanticsVerified, false);
...
test("Nova variations keep external IDs first class", () => {
  const evidence = normalizeNovaProduct({
    id: "p-1",
    name: "Variation product",
    sku: "PARENT",
    variations: [
      { id: "v-42", sku: "SIZE-42", barcode: "4000000000042", mpn: "M-42", manage_stock: true, in_stock: true, stock_status: "instock", stock_quantity: 2 }
    ]
  }, 2);
  const payload = evidence.normalizedPayload as Record<string, unknown>;
  const variants = payload.variants as Array<Record<string, unknown>>;
