...
test("Partner Network migration preserves anti-recruitment, RLS and fee-revenue boundaries", () => {
  const migration = readFileSync("db/migrations/0224_partner_network_foundation.sql", "utf8");
  ...
  const manifest = JSON.parse(readFileSync("db/migrations/checksums.0224.json", "utf8")) as Record<string, string>;
  const hash = createHash("sha256").update(migration).digest("hex");
  assert.equal(manifest["0224_partner_network_foundation.sql"], hash);
});
