...
export function myDataConfigFromEnv(env: NodeJS.ProcessEnv = process.env): MyDataConfig {
  const environment: MyDataEnvironment = env.AADE_MYDATA_ENVIRONMENT === "production" ? "production" : env.AADE_MYDATA_ENVIRONMENT === "test" ? "test" : (() => { throw new Error("AADE_MYDATA_ENVIRONMENT must be test or production"); })();
  ...
  const baseUrl = explicitBase || (environment === "production" ? "https://mydatapi.aade.gr/myDATA" : "");
  if (!baseUrl) throw new Error("AADE_MYDATA_BASE_URL is required for test mode so the deployment follows the currently assigned AADE test endpoint");
  if (!/^https:\/\//i.test(baseUrl)) throw new Error("AADE_MYDATA_BASE_URL must be HTTPS");
  const specVersion = env.AADE_MYDATA_SPEC_VERSION?.trim() || (environment === "production" ? "2.0.1" : "2.0.2");
  return { environment, baseUrl: baseUrl.replace(/\/+$/, ""), userId, subscriptionKey, requestTimeoutMs: positiveInt(env.AADE_MYDATA_REQUEST_TIMEOUT_MS, 15_000, "AADE_MYDATA_REQUEST_TIMEOUT_MS"), specVersion };
}
...
export class AadeMyDataClient {
  ...
  async requestDocs(input: { mark?: string; dateFrom?: string; dateTo?: string; entityVatNumber?: string; nextPartitionKey?: string; nextRowKey?: string } = {}): Promise<string> {
    const q = query(input);
    return this.#request(`RequestDocs${q ? `?${q}` : ""}`, { method: "GET" });
  }
