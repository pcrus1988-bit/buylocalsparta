...
import { ResendEmailProvider, ResendWebhookVerifier } from "../src/index.ts";
import type { Notification } from "@buy-local-sparta/core";

const notification:Notification={id:"ntf-1",userId:"user-1",channel:"email",purpose:"transactional",eventType:"order.confirmed",templateVersion:"v1",locale:"el",title:"Η παραγγελία σου",body:"Επιβεβαιώθηκε",payload:{},status:"queued",deliveryAttempts:0,createdAt:1};

test("sends transactional email with provider idempotency",async()=>{
  ...
  const body=JSON.parse(String(request?.init?.body));assert.equal(body.to[0],"customer@example.com");assert.equal(body.text,"Επιβεβαιώθηκε");
});
