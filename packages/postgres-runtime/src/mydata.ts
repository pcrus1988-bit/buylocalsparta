...
export class PostgresMyDataService {
  ...
  async transmitPreparedDocument(principal:SessionPrincipal,input:{documentId:string;now?:number}):Promise<MyDataTransmissionResult>{
    ...
    try{result=await this.#client.sendInvoices(prepared.xml);}catch(error){const message=error instanceof Error?error.message:String(error);await this.#uow.withTransaction(platformScope(principal.userId),async tx=>{await tx.query(`UPDATE mydata_transmission_attempts SET status='manual_review',http_summary=$3,completed_at=$4 WHERE tax_document_id=$1 AND attempt_key=$2`,[prepared.documentUuid,prepared.attemptKey,message.slice(0,1000),new Date(now)]);await tx.query(`UPDATE tax_documents SET transmission_status='manual_review',last_error=$2,last_transmission_at=$3 WHERE id=$1`,[prepared.documentUuid,message.slice(0,1000),new Date(now)]);});throw new Error("AADE myDATA outcome is uncertain; automatic retry is blocked pending reconciliation");}
    await this.#uow.withTransaction(platformScope(principal.userId),async tx=>{const first=result.items[0];const accepted=result.ok&&Boolean(first?.invoiceMark);const errors=result.items.flatMap(i=>i.errors.map(e=>e.code?`${e.code}: ${e.message}`:e.message)).join(" | ");await tx.query(`UPDATE mydata_transmission_attempts SET status=$3,response_snapshot=$4::jsonb,completed_at=$5 WHERE tax_document_id=$1 AND attempt_key=$2`,[prepared.documentUuid,prepared.attemptKey,accepted?"accepted":"rejected",JSON.stringify({ok:result.ok,items:result.items}),new Date(now)]);await tx.query(`UPDATE tax_documents SET transmission_status=$2,status=$3,aade_mark=$4,aade_uid=$5,aade_qr_url=$6,provider='aade_mydata_erp',provider_document_id=COALESCE($4,provider_document_id),last_error=$7,last_transmission_at=$8,issued_at=CASE WHEN $2='accepted' THEN COALESCE(issued_at,$8) ELSE issued_at END WHERE id=$1`,[prepared.documentUuid,accepted?"accepted":"rejected",accepted?"issued":"rejected",first?.invoiceMark??null,first?.invoiceUid??null,first?.qrUrl??null,errors||null,new Date(now)]);},{isolation:"serializable"});
    return result;
  }
}
