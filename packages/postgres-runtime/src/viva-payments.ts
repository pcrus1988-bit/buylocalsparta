...
export class PostgresVivaPaymentsService {
  ...
  async executeApprovedReturnRefund(input:{returnId:string;actorUserId:string;now?:number}):Promise<VivaRefundState>{
    ...
    await this.#uow.withTransaction({actorUserId:input.actorUserId,marketId:"sparta",platformAccess:true},async(tx)=>{const actor=await tx.query<SqlRow>(`SELECT id::text AS id FROM users WHERE public_id=$1 OR id::text=$1`,[input.actorUserId]);if(!actor.rowCount)throw new Error("Admin actor not found");const refundRow=await tx.query<SqlRow>(`SELECT id::text AS id FROM refunds WHERE public_id=$1`,[refund.id]);if(!refundRow.rowCount)throw new Error("Refund persistence not found");for(const line of prepared.lines)await tx.query(`UPDATE order_lines SET refunded_quantity=LEAST(quantity,refunded_quantity+$2),status=CASE WHEN refunded_quantity+$2>=quantity THEN 'refunded' ELSE status END WHERE id=$1`,[line.lineUuid,line.quantity]);await tx.query(`UPDATE return_lines SET refund_id=$2 WHERE return_id=$1`,[prepared.returnUuid,text(refundRow.rows[0].id,"refund_uuid")]);await tx.query(`UPDATE returns SET status='refunded',closed_at=$2,updated_at=$2 WHERE id=$1`,[prepared.returnUuid,new Date(now)]);await tx.query(`UPDATE customer_orders SET status=CASE WHEN NOT EXISTS (SELECT 1 FROM order_lines WHERE order_id=customer_orders.id AND status<>'cancelled' AND refunded_quantity<quantity) THEN 'refunded' ELSE 'partially_refunded' END,updated_at=$2 WHERE public_id=$1`,[prepared.orderId,new Date(now)]);await tx.query(`INSERT INTO audit_events(id,public_id,market_id,actor_user_id,actor_role,action,entity_type,entity_id,reason,after_state,created_at) VALUES($1,$2,(SELECT id FROM markets WHERE code='sparta'),$3,'platform_finance','return.refund.executed','return',$4,$5,$6::jsonb,$7)`,[randomUUID(),id("audit"),text(actor.rows[0].id,"actor_uuid"),prepared.returnId,`Viva refund ${refund.providerRefundId??refund.id}`,JSON.stringify({refundId:refund.id,amountMinor:refund.amountMinor,providerRefundId:refund.providerRefundId}),new Date(now)]);},{isolation:"serializable"});
    return refund;
  }
  ...
  async #reconcileReversal(envelope:VivaWebhookEnvelope,reversalTransactionId:string,now:number):Promise<VivaRefundState>{
    ...
    return this.#uow.withTransaction({marketId:"sparta",platformAccess:true},async(tx)=>{
      ...
      await tx.query(`UPDATE payments SET refunded_minor=$2,status=CASE WHEN $2>=captured_minor THEN 'refunded' ELSE 'partially_refunded' END,provider_verified_at=$3,updated_at=$3 WHERE id=$1`,[paymentUuid,next,new Date(now)]);
