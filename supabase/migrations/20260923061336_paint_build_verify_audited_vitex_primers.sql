-- Initial audited VITEX primer verification migration.
-- Kept verbatim for production migration-history parity. This pass matched uppercase VITEX;
-- production manufacturer values use title case, corrected by the following migration.
update public.manufacturer_products mp
set verification_status='verified', last_verified_at=now(), updated_at=now()
where mp.product_name in ('Anti-Rust Primer','Blanco Eco','Durovit','Primer 100% Acrylic')
  and mp.manufacturer='VITEX'
...
where ap.product_id=mp.id and ap.is_current=true and ap.source_layer='manufacturer'
  and mp.product_name in ('Anti-Rust Primer','Blanco Eco','Durovit','Primer 100% Acrylic') and mp.manufacturer='VITEX' and mp.verification_status='verified';
