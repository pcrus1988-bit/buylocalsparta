...
export class CatalogCrawlerStore {
  ...
  async claim(workerId: string, leaseSeconds: number): Promise<ClaimedCrawlJob | undefined> {
    const result = await this.#db.query<SqlRow>(
      `SELECT * FROM bls_private.claim_catalog_web_crawl_job($1,$2)`,
    ...
    return {
      jobId: required(row.job_id, "job_id"),
      profileId: required(row.profile_id, "profile_id"),
      sourceId: required(row.source_id, "source_id"),
      crawlMode,
      seedUrl: optional(row.seed_url),
      policySnapshot: row.policy_snapshot,
      extractorVersion: required(row.extractor_version, "extractor_version"),
      attemptCount: integer(row.attempt_count, "attempt_count")
    };
  }

  async renew(jobId: string, workerId: string, leaseSeconds: number): Promise<void> {
  ...
  async markFetching(pageId: string, robotsAllowed: boolean | null): Promise<void> {
    await this.#db.query(`
      ...
      SET status='fetching',robots_allowed=$2,updated_at=now(),failure_kind=NULL,failure_reason=NULL
      WHERE id=$1 AND status IN ('queued','failed')
    `, [pageId, robotsAllowed]);
  }

  async markSkipped(pageId: string, reason: string, robotsAllowed: boolean | null): Promise<void> {
    await this.#db.query(`
      UPDATE public.catalog_web_crawl_pages
      SET status='skipped',skip_reason=$2,robots_allowed=$3,extraction_status='not_applicable',updated_at=now()
      WHERE id=$1
    `, [pageId, reason.slice(0, 1000), robotsAllowed]);
  }

  async markFailed(pageId: string, kind: string, reason: string): Promise<void> {
    await this.#db.query(`
      UPDATE public.catalog_web_crawl_pages
      SET status='failed',failure_kind=$2,failure_reason=$3,extraction_status='failed',updated_at=now()
      WHERE id=$1
    `, [pageId, kind.slice(0, 100), reason.slice(0, 2000)]);
  }

  async markFetched(input: {
    pageId: string;
    result: SecureCrawlFetchResult;
    productLikelihood: number;
    extractionStatus: "not_applicable" | "extracted" | "review_required";
  }): Promise<void> {
    await this.#db.query(`
      UPDATE public.catalog_web_crawl_pages
      SET status='fetched',url=$2,normalized_url=$2,robots_allowed=COALESCE(robots_allowed,true),
          resolved_addresses=($3::text[])::inet[],fetch_mode='http',http_status=$4,
          content_type=$5,response_bytes=$6,response_sha256=$7,etag=$8,last_modified_header=$9,
          redirect_chain=$10::jsonb,product_likelihood=$11,extraction_status=$12,
          fetched_at=now(),failure_kind=NULL,failure_reason=NULL,updated_at=now()
      WHERE id=$1
    `, [
      input.pageId,
      input.result.finalUrl,
      [...input.result.resolvedAddresses],
      input.result.status,
      input.result.headers["content-type"] ?? null,
      input.result.responseBytes,
      input.result.responseSha256,
      input.result.headers.etag ?? null,
      input.result.headers["last-modified"] ?? null,
      JSON.stringify(input.result.redirectChain),
      input.productLikelihood,
  ...
  async saveExtraction(input: { pageId: string; extractorVersion: string; ordinal: number; candidate: ExtractedProductCandidate }): Promise<"accepted" | "review_required"> {
    ...
    await this.#db.query(`
      INSERT INTO public.catalog_web_product_extractions(
        page_id,extraction_version,ordinal,source_product_key,status,confidence,
        extracted_payload,field_provenance,quality_payload,accepted_at
      )
      ...
      SET source_product_key=EXCLUDED.source_product_key,
    ...
    `, [
      ...
      input.ordinal,
      input.candidate.sourceProductKey,
      status,
      confidence,
      JSON.stringify(input.candidate),
      JSON.stringify(input.candidate.fieldEvidence),
      JSON.stringify({ valid: quality.valid, normalizedGtin: quality.normalizedGtin ?? null, issues: quality.issues })
    ]);
    return status;
  }
...
function extractionConfidence(candidate: ExtractedProductCandidate): number {
  const values: number[] = [];
  for (const evidence of Object.values(candidate.fieldEvidence)) {
    for (const item of Array.isArray(evidence) ? evidence : [evidence]) values.push(item.confidence);
  }
